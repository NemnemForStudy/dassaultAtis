...
<body>
	...
	JPO.invoke(context, "decInterfaceDV", null, "getDeliverableStatus", null , null);
	//getVendorPrint
...
// 	}
	...
	<script>
		...
		var nineDaysAgo = new Date(); // 9일 전의 날짜를 담을 변수
		nineDaysAgo.setDate(now_utc.getDate() - 9); // 9일을 뺀 날짜 설정
		console.log(nineDaysAgo);
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		renderChart(data, dataString) // 페이지 로드됐을때 방문자 그래프 작성
			 ...
			 $.ajax({
			    ...
			    success: function(response) {
			       ...
			       // newData.push(abc); // 
			        ...
			        renderChart(newData, newCategories)
			      },
		...
		function renderChart(data, categoriesDate) {
