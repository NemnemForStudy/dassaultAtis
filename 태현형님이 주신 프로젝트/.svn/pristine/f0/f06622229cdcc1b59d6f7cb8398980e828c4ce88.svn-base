...
try
{
	...
	for(int i = 0 ; i < columnRows ; i++){
		...
		if (lastIndex != -1 && lastIndex + 1 < row.length()) {
		    ...
		    String gChangeLogNumber = String.valueOf(mObject.get("CHANGELOGNUMBER"));
		    
		    Map selectParamMap = new HashMap();
			selectParamMap.put("CHANGELOGNUMBER",gChangeLogNumber);
			selectParamMap.put("SITE_CD",siteCd);

			 try ( SqlSession sqlSession = decSQLSessionFactory.getBatchSession() ){
					
						sqlSession.delete("Project.deleteProjectChangeLog", selectParamMap);
						System.out.println("*****Delete ProjectChangeHistory Finish****");
						sqlSession.commit();
