...
//End:R207:PRG Bug :369101
    ...
    if(calledMethod.equalsIgnoreCase("submitDeleteTask")){
        ...
        String[] sArrselectedTaskIds = selectedTaskIds.split("\\|");
        StringList slParam = new StringList(DecConstants.SELECT_ATTRIBUTE_DECWBSTYPE);
        ...
        slParam.add(DecConstants.SELECT_TYPE);
  		try( SqlSession sqlSession = decSQLSessionFactory.getSession() ){
	        BusinessObjectWithSelectList bowsl = ProgramCentralUtil.getObjectWithSelectList(context, sArrselectedTaskIds, slParam);
    		Map mWhere = new HashMap();
	        for(BusinessObjectWithSelect bows : bowsl){
	        	String sType = bows.getSelectData(DecConstants.SELECT_TYPE);
	        	if(!sType.equals(DecConstants.TYPE_DECIWPTASK)){
					if(!PersonUtil.hasAssignment(context, "decSystemAdmin")){
						...
						if(!Boolean.valueOf(hasAccessEpxr)) {
			        		%>
			         	   	<script language="javascript" type="text/javaScript">
			         	   		alert("<framework:i18nScript localize="i18nId">ProgramCentral.Alert.IsNotHaveRole</framework:i18nScript>");
			         	   	</script>
			                <%
			                return;
						}
	        	...
	        	String sWBSType = bows.getSelectData(DecConstants.SELECT_ATTRIBUTE_DECWBSTYPE);
	        	String sUnitOID = bows.getSelectData(DecConstants.SELECT_ID);
	        	if("Unit".equals(sWBSType)){
	        		mWhere.put("UNIT_ID", sUnitOID);
	        		mWhere.put("SITE_CD", doPS.getName());
	            	List<Map> lReturn = sqlSession.selectList("Project.selectConstructionKPI", mWhere);
	            	if(!lReturn.isEmpty()){
		        		%>
		         	   	<script language="javascript" type="text/javaScript">
		         	   		alert("<framework:i18nScript localize="i18nId">ProgramCentral.Alert.IsNotDeleteUnitHaveKPI</framework:i18nScript>");
		         	   	</script>
		                <%
		                return;
	            	}
...
<%
 ...
 //End:24-june-2010:s4e:R210 PRG:WBSEnhancement
	// jhlee Add 2023-08-01 첨부파일 보고서
	<%
	if(calledMethod.equals("submitReferenceDocumentReport") ){
	%>
		function submitReferenceDocumentReport(projId,selectedIds,count,projectSelected) {
		    var showAll = false;
		    var selectedTaskIds = "";
		     if(projectSelected == 'true'){
		        showAll = true;
		    ...
		    if(selectedIds == "" && projectSelected == 'false'){
		      alert("<framework:i18nScript localize="i18nId">emxProgramCentral.Project.SelectTaskBeforeSubmitting</framework:i18nScript>");
		        ...
		        String strFinalList = slFinalList.toString().substring(1,slFinalList.toString().length()-1);
		        %>
		        var reportURL ="../common/emxIndentedTable.jsp?table=decProjectReferenceDocumentReportSummary&freezePane=TaskName&suiteKey=<%=suiteKey%>&header=ProgramCentral.Command.decWBSReferenceDocumentsReport&calculations=false&HelpMarker=emxhelpdeliverablesreport&program=emxProjectReport:getProjectWBSDeliverableList&taskIds=<%=strFinalList%>&objectId="+projId+"&showAll="+showAll+"&sortColumnName=WBS&showPageURLIcon=false";
		     showModalDialog(reportURL, 930,650, true);
		    }
		}
	<%
