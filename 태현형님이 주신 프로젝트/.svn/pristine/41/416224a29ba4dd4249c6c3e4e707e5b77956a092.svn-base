...
String sReportFormatLabel = EnoviaResourceBundle.getProperty(context, "emxEngineeringCentralStringResource", context.getLocale(),"emxEngineeringCentral.ECBOMCompare.ReportFormat");
	...
	try {
		SCTableBean = (com.matrixone.apps.framework.ui.decUIStructureCompare)structureCompareBean;
		HashMap sessionValueMap = (HashMap)((com.matrixone.apps.framework.ui.decUIStructureCompare)SCTableBean).getSCCriteria(sTimeStamp);
		sessionValueMap.put("CriteriaHeader", exportSubHeader);
		((com.matrixone.apps.framework.ui.decUIStructureCompare)SCTableBean).setStructureCompareCriteria(sTimeStamp, sessionValueMap);
	} catch (Exception e) {
		e.printStackTrace();
	}
