...
/**
 * Code inspired and modified from springyui.js
 */

define("BTWWUtils/SpringyGraph_ES5_UM5_v1/SpringyGraph", ["BTWWLibrairies/springy/springy", "UWA/Drivers/jQuery"], function(Springy, jQuery) {
    var SpringyRenderer = function(options) {
        var graph = (this.graph = options.graph || new Springy.Graph());
        var nodeFont = "12px Verdana, sans-serif";
        var edgeFont = "8px Verdana, sans-serif";
        var stiffness = options.stiffness || 400.0;
        var repulsion = options.repulsion || 400.0;
        var damping = options.damping || 0.5;
        var minEnergyThreshold = options.minEnergyThreshold || 0.01;
        var minSpeedThreshold = options.minSpeedThreshold || 0.1;
        var nodeSelected = options.nodeSelected || null;
        var nodeImages = {};
        var edgeLabelsUpright = true;

        var onRenderStart = options.onRenderStart || null;
        var onRenderStop = options.onRenderStop || null;

        var canvas = options.canvas;
        var ctx = canvas.getContext("2d");

        var layout = (this.layout = new Springy.Layout.ForceDirected(graph, stiffness, repulsion, damping, minEnergyThreshold, null, minSpeedThreshold));
        ...
        var renderer = (this.renderer = new Springy.Renderer(
            ...
            function drawEdge(edge, p1, p2) {
                ...
                if (edge.data.label !== undefined) {
                    ...
                    var textPos = s1
                        .add(s2)
                        .divide(2)
                        .add(normal.multiply(displacement));
            ...
            function drawNode(node, p) {
                ...
                if (selected !== null && selected.node !== null && selected.node.id === node.id) {
                    ...
                    if (node.data.backcolor) {
                        ctx.fillStyle = node.data.backcolor;
                    } else {
            ...
            function onRenderStopFct() {
                if (typeof onRenderStop === "function") {
                    onRenderStop();
                }
            },
            function onRenderStartFct() {
    ...
    SpringyRenderer.Graph = Springy.Graph;
