...
forwardURL = forwardURL.replaceFirst("\\$", "?");
...
// Modified by hslee on 2023.07.03 --- [s]
// 	var openURLIdx = getTopWindow().name.indexOf("||");		
// 	if(openURLIdx >= 0){	    
// 		var openURL = getTopWindow().name.substring(openURLIdx+2);
// 	var openURLIdx = location.name.indexOf("||");	
// 	if(openURLIdx >= 0){	    
// 		var openURL = getTopWindow().name.substring(openURLIdx+2);
// Modified by hslee on 2023.07.03 --- [e]     
  		emxUINavigator.setContent("<%=forwardURL %>");
//  	}
 });
 //XSSOK
 ...
 // Added by hslee on 2023.07.03 --- [s]
 function fnOnload() {
	 document.getElementById("pageContentDiv").style.left = "0";
 }
 // Added by hslee on 2023.07.03 --- [e]
 </script>
</head>
<%
request.setAttribute("callerPage", "popup");
%>
<!-- Modified by hslee on 2023.07.03 -->
<!-- <body class="dialog navigator" onload="initializeSlideIns();expandMenuData();" > -->
<body class="dialog navigator" onload="initializeSlideIns();expandMenuData();fnOnload();" >
