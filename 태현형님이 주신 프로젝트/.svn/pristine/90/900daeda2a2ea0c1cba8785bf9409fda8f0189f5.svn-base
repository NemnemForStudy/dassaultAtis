...
<%
  ...
  try (SqlSession sqlSession = decSQLSessionFactory.getSession()){
		...
		List<Map> selectList = sqlSession.selectList("Project.selectProgressOne",insertParamMap);
		List<Map> selectList2 = sqlSession.selectList("Project.selectDailyProgressOne",insertParamMap);
		...
		if(selectList2.size() > 0){
			hm2 = (HashMap) selectList2.get(0);
		}
		
		if(!hm2.isEmpty()){
			Iterator<String> iter = hm2.keySet().iterator();
            
            while(iter.hasNext()) {
                String key = iter.next();
                String value = (String) hm2.get(key);
                map.put(key,value);
            }
		}
