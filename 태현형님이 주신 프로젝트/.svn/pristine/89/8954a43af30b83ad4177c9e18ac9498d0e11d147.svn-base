...
<%-- Common Includes --%>
<%@page import="com.dec.util.DecCommonUtil"%>
       ...
       <script language="javascript" type="text/javaScript">
		...
		<%
			...
			<%
    		   ...
    		   String[] projects = emxGetParameterValues(request,"emxTableRowIdActual");// Modified by thok 2023.08.09 emxTableRowId -> emxTableRowIdActual
...
<%
	...
	<%
	    		...
	    		if("0".equals(rowId) && stateList.contains(selectedObjectState)){
	    			...
	    			// Modified by jhlee on 2023-06-09 --- [s] &preProcessJavaScript=changeDurationAndPolicy 파라미터 제거
	    			String strURL = DecCommonUtil.getCreateTaskURL(context, objectId, objId, parentId, rowId, portalCommandName);
	    			// Modified by jhlee on 2023-06-09 --- [e]
		    		%>
		        	<script language="javascript">
			        	if(parent.emxEditableTable.checkDataModified()){
			                alert("<framework:i18nScript localize="i18nId">emxProgramCentral.Common.SaveWBSModification</framework:i18nScript>");
...
<%
			...
			String strErrorMsg = EnoviaResourceBundle.getProperty(context, "ProgramCentral",
					"emxProgramCentral.ProjectTemplate.CannotCreateTemplateStructure", context.getSession().getLanguage());
			%><script language="javascript" type="text/javaScript">
			{
				alert("<%= strErrorMsg %>");
			}
...
<%
	...
	<%
        ...
        //  String[] methodArgs = JPO.packArgs(paramMap);
        //  Modified by jhlee on 2023-06-09 --- [s] editLink 파라미터 제거
        //  boolean hasModifyAccess  = (boolean) JPO.invoke(context,"emxProjectMember", null, "hasAccessToModifyProject", methodArgs, Boolean.class);
      	...
      	//		strURL = strURL + "&editLink=true";
      	//	}
      	//  Modified by jhlee on 2023-06-09 --- [e] editLink 파라미터 제거
      		%>
            <script language="javascript">
          		 var url = "<%=strURL%>";
          		 document.location.href = url;
    ...
    <%
		...
		return;
            ...
            <%
	        	...
	        	String exportFormat = "";
	        	String errorMessage ="";
	        	String selectedNodeId = emxGetParameter(request, "emxTableRowIdActual");// Modified by thok 2023.08.09 emxTableRowId -> emxTableRowIdActual
