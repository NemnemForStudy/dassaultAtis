...
<%
	...
	try {
		for(int i=0; i<rwsl.size(); i++) {
			...
			String sEmpty = emxGetParameter(request, sPersonOID+"_empty");
			if("true".equals(sEmpty)){
				sbParam.setLength(0);
				sbParam.append("mod connection $1 $2 $3");
				List<String> lParam = new ArrayList();
				lParam.add(slMemberRELID.get(i));
				lParam.add(DecConstants.ATTRIBUTE_DECPROJECTMEMBERROLE);
				lParam.add("");
