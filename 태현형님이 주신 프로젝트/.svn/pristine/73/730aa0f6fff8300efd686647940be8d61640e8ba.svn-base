...
<%
  ...
  String deleteErrMsg = ProgramCentralConstants.EMPTY_STRING;
  ...
  if(strInvokedFrom != null && "StructureBrowser".equals(strInvokedFrom)) {
	  ...
	  //Modified by thok on 2023.05.12 --- [S]
	  for (int i=0;i<allProjectsToDelete.length;i++)
	  {
		  
	      StringList selectable = new StringList();
		  selectable.addElement("from[decProjectExecutionRel]");
		  selectable.addElement("to[Project Access List].from.from[Project Access Key]");
		  selectable.addElement(DecConstants.SELECT_ATTRIBUTE_DECPROJECTTYPE);
		  selectable.addElement(ProgramCentralConstants.SELECT_CURRENT);
	      DomainObject deleteObject = DomainObject.newInstance(context,allProjectsToDelete[i]);
	      
		  Map <String,String> deleteObjectInfo = deleteObject.getInfo(context, selectable);
		  if("true".equalsIgnoreCase(deleteObjectInfo.get("from[decProjectExecutionRel]"))){
			  deleteErrMsg = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.Delete.errMsg2", languageStr);
			  break;
		  }else if("true".equalsIgnoreCase(deleteObjectInfo.get("to[Project Access List].from.from[Project Access Key]"))){
			  deleteErrMsg = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.Delete.errMsg1", languageStr);
			  break;
		  }else if("ongoing".equalsIgnoreCase(deleteObjectInfo.get(DecConstants.SELECT_ATTRIBUTE_DECPROJECTTYPE)) 
				  && !"create".equalsIgnoreCase(deleteObjectInfo.get(ProgramCentralConstants.SELECT_CURRENT))){
			  deleteErrMsg = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.Delete.errMsg3", languageStr);
			  break;
		  }
	  }
	  if(deleteErrMsg.length() > 0){
		   %>
	        <script type="text/javascript" language="javascript">
				alert('<%=XSSUtil.encodeForJavaScript(context,deleteErrMsg)%>');	
			</script>
	          <%
		   return;
	   }
  ...
  if ( projects != null && projects.length > 0) {
  	...
  	for (int i=0; numProjects>i; i++) {
		...
		if(deleteProj == true) { 
			...
			// Master Project Schedule
			// End - Master Project Schedule
			// Modified by hslee on 2023.09.18 --- [s]
// 			try
			try (SqlSession sqlSession = decSQLSessionFactory.getSession())
			{
				...
				mParam.put("projectId", project.getId());
				String sResult = JPO.invoke(context, "decCodeMaster", null, "deleteProjectCodeMaster", JPO.packArgs(mParam), String.class);
				
				mParam.put("sqlSession", sqlSession);
				Map resultMap = JPO.invoke(context, "decKeyQty", null, "deleteProjectKeyQty", JPO.packArgs(mParam), Map.class);
				...
				ContextUtil.commitTransaction(context);
				sqlSession.commit(); 
				// Added by hslee on 2023.09.18 --- [e]
			} catch(Exception ex) {
