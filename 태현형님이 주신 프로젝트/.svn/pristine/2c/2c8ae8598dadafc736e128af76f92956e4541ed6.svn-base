...
<%!
public String getColumnLabel(boolean hasTableMaster, String strColumnName, Map tableMap, Map hmpColumnData) throws Exception {
// 	try {
		String strColumnLabel = null;
		if ( hasTableMaster )
		{
			String columnNameWithoutUser = null;
			if ( strColumnName.indexOf("@") > -1 )
			{
				columnNameWithoutUser = strColumnName.substring(strColumnName.indexOf("@") + 1, strColumnName.length());
			}
			else
			{
				columnNameWithoutUser = strColumnName;
			}
			Map columnMap = (Map) tableMap.get(columnNameWithoutUser);
			if ( columnMap != null )
			{
				strColumnLabel = (String) columnMap.get(DecConstants.SELECT_DESCRIPTION);
			}
		}
		
		if ( StringUtils.isEmpty(strColumnLabel) )
		{
...
catch(FrameworkException e){ // 보안 취약 조치
...
<%
        ...
        String objectId                = emxGetParameter(request,"objectId"); // Added by hslee on 2023.08.03
        ...
        try
        {
            ...
            if(!"".equalsIgnoreCase(strUIType) && (strUIType!=null))
            {
                ...
                // Modified by choimingi on 2023.11.03 --- [s] --> 
                if(DecStringUtil.contains(strCurrentTable, "DeliverableStatusTable") || DecStringUtil.contains(strCurrentTable, "VPDocumentStatusTable")){
               	 	mplAttributeColumns.clear();
               	}
                // Modified by choimingi on 2023.11.03 --- [e] -->
...
<%
    ...
    if ( StringUtils.isNotEmpty(objectId) )
    {
    	...
    	if ( DecConstants.TYPE_PROJECT_SPACE.equals(type) )
    	{
    		...
    		String tableName = strCurrentTable;
    		if ( strCurrentTable.indexOf("~") > -1 )
    		{
    			tableName = strCurrentTable.substring(strCurrentTable.indexOf("~") + 1, strCurrentTable.length());
    		}
    		
    		Map programMap = new HashMap();
    		programMap.put("projectId", objectId);
    		programMap.put("codeMasterName", tableName);
//    		programMap.put("codeMasterRevision", tableMasterRevision);
    		...
    		tableMap = decListUtil.getSelectKeyDataMapForMapList(tableDetailList, DecConstants.SELECT_ATTRIBUTE_DECCODE);
    		hasTableMaster = true;
    	}
    }
        ...
        <%
                    if((mplHiddenColumns != null)&& (mplHiddenColumns.size()>0))
                    {
                        for (int i = 0, size = mplHiddenColumns.size(); i < size; i++)
                        {
                            ...
                            // Modified by hslee on 2023.08.03 - Table Master 적용
//                             strColumnText   = (String) hiddenMap.get("displayLabel");
							strColumnText   = getColumnLabel(hasTableMaster, strColumnName, tableMap, hiddenMap);
...
//                             strColumnText   = (String) basicMap.get("displayLabel");
							if(tableMap!=null){ // 보안 취약 조치
                            	strColumnText   = getColumnLabel(hasTableMaster, strColumnName, tableMap, basicMap);
                            }
...
//                             strColumnLabel      = (String) expressionMap.get("displayLabel");
                            if(tableMap!=null){ // 보안 취약 조치
                            strColumnLabel   = getColumnLabel(hasTableMaster, strColumnName, tableMap, expressionMap);
                            }
...
//                             strColumnLabel      = (String) attributeMap.get("displayLabel");
                            if(tableMap!=null){ // 보안 취약 조치
								strColumnLabel   = getColumnLabel(hasTableMaster, strColumnName, tableMap, attributeMap);
                            }
...
//                             strColumnLabel  = (String) relAttrMap.get("displayLabel");
                            if(tableMap!=null){ // 보안 취약 조치
								strColumnLabel   = getColumnLabel(hasTableMaster, strColumnName, tableMap, relAttrMap);
                            }
...
<%
            for (int i = 0, size = mplVisibleColumns.size(); i < size; i++)
            {
                ...
                // Modified by hslee on 2023.08.03 --- [s]
//                 strColumnLabel          = (String) hmpColumnData.get("displayLabel");
                strColumnLabel   = getColumnLabel(hasTableMaster, strColumnName, tableMap, hmpColumnData);
				// Modified by hslee on 2023.08.03 --- [e]
