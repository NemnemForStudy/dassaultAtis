...
/**
 * The <code>emxIssueBase</code> class contains methods related to Issue
 * Management.
 * 
...
public class emxCommonIssueBase_mxJPO extends emxDomainObject_mxJPO {
	...
	/**
	 * A string constant with the value
	 * emxComponents.Form.Radio.LatestReleasedRevisionOnly.
	 */
	...
	/**
	 * A string constant with the value "<img
	 * src=\"../common/images/iconStatusGreen.gif\" border=\"0\" align=\"middle\">".
	 */
	public static final String GREEN_ICON = "<img src=\"../common/images/iconStatusGreen.gif\" border=\"0\"  align=\"middle\">";
	/**
	 * A string constant with the value "<img
	 * src=\"../common/images/iconStatusRed.gif\" border=\"0\" align=\"middle\">".
	 */
	public static final String RED_ICON = "<img src=\"../common/images/iconStatusRed.gif\" border=\"0\"  align=\"middle\">";
	/**
	 * A string constant with the value "<img
	 * src=\"../common/images/iconStatusYellow.gif\" border=\"0\"
	 * align=\"middle\">".
	 */
	...
	protected static final String POLICY_VERSIONED_DESIGN_POLICY = PropertyUtil
			.getSchemaProperty("policy_VersionedDesignPolicy");
	...
	public int mxMain(Context context, String[] args) throws Exception {
		if (!context.isConnected()) {
			String strContentLabel = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
	...
	public MapList getAllIssues(Context context, String[] args) throws Exception {
		...
		String strRelationAssignedIssue = PropertyUtil.getSchemaProperty(context,
		...
		sbOwnerCondition.append(SYMB_OPEN_PARAN).append(SYMB_OPEN_PARAN).append(DomainConstants.SELECT_OWNER)
				.append(SYMB_EQUAL).append(SYMB_QUOTE).append(strOwner).append(SYMB_QUOTE).append(SYMB_CLOSE_PARAN)
				.append(SYMB_OR).append(SYMB_OPEN_PARAN).append(DomainConstants.SELECT_ORIGINATOR).append(SYMB_EQUAL)
				.append(SYMB_QUOTE).append(strOwner).append(SYMB_QUOTE).append(SYMB_CLOSE_PARAN).append(SYMB_OR);
		// Added to check if Assigned Issue present or not
		sbOwnerCondition.append(SYMB_OPEN_PARAN).append(SYMB_TO).append(SYMB_OPEN_BRACKET)
				.append(strRelationAssignedIssue).append(SYMB_CLOSE_BRACKET).append(SYMB_EQUAL).append("TRUE")
				.append(SYMB_CLOSE_PARAN).append(SYMB_AND);
		// Added to include the Assigned Issues in ALL Filter.
		sbOwnerCondition.append(SYMB_OPEN_PARAN).append(SYMB_TO).append(SYMB_OPEN_BRACKET)
				.append(strRelationAssignedIssue).append(SYMB_CLOSE_BRACKET).append(SYMB_DOT).append(SYMB_FROM)
				.append(SYMB_DOT).append(DomainConstants.SELECT_NAME).append(SYMB_EQUAL).append(SYMB_QUOTE)
				.append(strOwner).append(SYMB_QUOTE).append(SYMB_CLOSE_PARAN);
		// Added to include Co-Owners Condition in where clause //
		sbOwnerCondition.append(SYMB_OR).append("( ").append(SYMB_QUOTE).append(context.getUser()).append(SYMB_QUOTE)
				.append(" matchlist ").append(" attribute[" + DomainConstants.ATTRIBUTE_CO_OWNER + "] ")
				.append(" \"~\" )");
		// End Added to include Co-Owners Condition in where clause //
		sbOwnerCondition.append(SYMB_CLOSE_PARAN).append(SYMB_AND).append(SYMB_OPEN_PARAN)
				.append(DomainConstants.SELECT_CURRENT).append(SYMB_NOT_EQUAL).append(SYMB_QUOTE).append(strClose)
				.append(SYMB_QUOTE).append(SYMB_CLOSE_PARAN);
		...
		// the number of levels to expand, 1 equals expand one level, 0 equals expand
		// all
		...
		try {
			...
			relIssueList = (MapList) domObj.getRelatedObjects(context, strRelFoundIn, strType, objectSelects, null,
					false, true, sRecurseToLevel, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
		} catch (Exception e) {
	...
	public MapList getAssignedIssues(Context context, String[] args) throws Exception {
		...
		sbOwnerCondition.append(SYMB_OPEN_PARAN).append(SYMB_TO).append(SYMB_OPEN_BRACKET)
				.append(strRelationAssignedIssue).append(SYMB_CLOSE_BRACKET).append(SYMB_DOT).append(SYMB_FROM)
				.append(SYMB_DOT).append(DomainConstants.SELECT_NAME).append(SYMB_EQUAL).append(SYMB_QUOTE)
				.append(context.getUser()).append(SYMB_QUOTE).append(SYMB_CLOSE_PARAN).append(SYMB_AND)
				.append(SYMB_OPEN_PARAN).append(DomainConstants.SELECT_CURRENT).append(SYMB_NOT_EQUAL)
				.append(SYMB_QUOTE).append(strClose).append(SYMB_QUOTE).append(SYMB_CLOSE_PARAN);
	 ...
	 * @since AEF 10.0.5.0
	 */
	protected MapList getDesktopIssues(Context context, String strWhereCondition, String strOwnerCondition)
			throws Exception {
		...
		MapList mapBusIds = findObjects(context, strType, null, null, strOwnerCondition, DomainConstants.QUERY_WILDCARD,
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries: ObjectId -
	 *                The object Id of the context.
	...
	public MapList getAllContextIssues(Context context, String[] args) throws Exception {
		 ...
		 * Issue.SYMBOLIC_type_Issue); //getting the symbolic relationship from
		 * PropertyUtil //String strRelFoundIn = RELATIONSHIP_ISSUE; String
		 * strRelFoundIn = PropertyUtil.getSchemaProperty( context,
		 * Issue.SYMBOLIC_relationship_Issue); //Initializing the return type MapList
		 * relIssueList = null; //Business Objects are selected by its Ids StringList
		 * objectSelects = new StringList(DomainConstants.SELECT_ID);
		 * objectSelects.add("current.access[modify]"); //Relationships are selected by
		 * its Ids StringList relSelects = new
		 * StringList(DomainConstants.SELECT_RELATIONSHIP_ID); //the number of levels to
		 * expand, 1 equals expand one level, 0 equals expand all short sRecurseToLevel
		 * = 1; //retrieving Issue List from Context this.setId(strParentId);
		 * relIssueList = getRelatedObjects( context, strRelFoundIn, strType,
		 * objectSelects, relSelects, true, false, sRecurseToLevel,
		 * DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING); return
		 * relIssueList;
		 */
	...
	public MapList getClosedIssues(Context context, String[] args) throws Exception {
		 ...
		 * 
		 * String strClosedCondition = ""; //StringBuffer variable for where expression
		 * StringBuffer sbClosedCondition = new StringBuffer(20);
		 * 
		 * //Forming the where expression sbClosedCondition.append(SYMB_OPEN_PARAN);
		 * sbClosedCondition.append(DomainConstants.SELECT_CURRENT);
		 * sbClosedCondition.append(SYMB_EQUAL); sbClosedCondition.append(SYMB_QUOTE);
		 * sbClosedCondition.append(strClose); sbClosedCondition.append(SYMB_QUOTE);
		 * sbClosedCondition.append(SYMB_CLOSE_PARAN); strClosedCondition =
		 * sbClosedCondition.toString();
		 * 
		 * //getting the symbolic relationship from PropertyUtil //String strRelFoundIn
		 * = RELATIONSHIP_ISSUE; String strRelFoundIn = PropertyUtil.getSchemaProperty(
		 * context, Issue.SYMBOLIC_relationship_Issue);
		 * 
		 * //Initializing the return type MapList relIssueList = null; //Business
		 * Objects are selected by its Ids StringList objectSelects = new
		 * StringList(DomainConstants.SELECT_ID);
		 * objectSelects.add("current.access[modify]"); //Relationships are selected by
		 * its Ids StringList relSelects = new
		 * StringList(DomainConstants.SELECT_RELATIONSHIP_ID); //the number of levels to
		 * expand, 1 equals expand one level, 0 equals expand all short sRecurseToLevel
		 * = 1; //retrieving Issue from Context this.setId(strParentId); relIssueList =
		 * getRelatedObjects( context, strRelFoundIn, strType, objectSelects,
		 * relSelects, true, false, sRecurseToLevel, strClosedCondition,
		 * DomainConstants.EMPTY_STRING); return relIssueList;
		 */
	...
	public MapList getActiveIssues(Context context, String[] args) throws Exception {
		...
		// To get actual policy Name
		/*
		 * String strIssuePolicy = PropertyUtil.getSchemaProperty( context,
		 * Issue.SYMBOLIC_policy_Issue);
		 * 
		 * //Getting the type name String strType =
		 * PropertyUtil.getSchemaProperty(context, Issue.SYMBOLIC_type_Issue); //String
		 * strProductType = ProductCentralDomainConstants.TYPE_PRODUCTS; String
		 * strClosedCondition = ""; //Getting and representing the state 'Close' String
		 * strClose = FrameworkUtil.lookupStateName(context, strIssuePolicy,
		 * SYMB_CLOSE); //StringBuffer variable to form the where expression
		 * StringBuffer sbClosedCondition = new StringBuffer(20);
		 * 
		 * //Forming the expression sbClosedCondition.append(SYMB_OPEN_PARAN);
		 * sbClosedCondition.append(DomainConstants.SELECT_CURRENT);
		 * sbClosedCondition.append(SYMB_NOT_EQUAL);
		 * sbClosedCondition.append(SYMB_QUOTE); sbClosedCondition.append(strClose);
		 * sbClosedCondition.append(SYMB_QUOTE);
		 * sbClosedCondition.append(SYMB_CLOSE_PARAN);
		 * 
		 * strClosedCondition = sbClosedCondition.toString(); //getting the symbolic
		 * relationship from PropertyUtil //String strRelFoundIn = RELATIONSHIP_ISSUE;
		 * String strRelFoundIn = PropertyUtil.getSchemaProperty( context,
		 ...
		 * = 1; //retrieving Issue List from Context this.setId(strParentId);
		 * relIssueList = getRelatedObjects( context, strRelFoundIn, strType,
		 * objectSelects, relSelects, true, false, sRecurseToLevel, strClosedCondition,
		 * DomainConstants.EMPTY_STRING); return relIssueList;
		 */
	 ...
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries: ObjectId -
	 *                The object Id of the context. relationName - The name of the
	...
	public MapList getAllRelatedResolvedTo(Context context, String[] args) throws Exception {
		...
		/*
		 * String strActualRelName = PropertyUtil.getSchemaProperty(context,
		 * relationshipName); RelationshipType relType = new
		 * RelationshipType(strActualRelName); BusinessTypeList BusTypeList = new
		 * BusinessTypeList(); BusTypeList = relType.getToTypes(context); String
		 * strTypes = ""; strTypes = FrameworkUtil.getTypesList(BusTypeList);
		 */
		String strRelationship = PropertyUtil.getSchemaProperty(context, relationshipName);
		MapList relBusObjPageList = getRelatedObjects(context, strRelationship, "*", objectSelects, relSelects, false,
				true, sh, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
...
//                return relBusObjPageList;
		...
		try {
			...
			relBusObjPageList = issObj.getAllAffectedItems(context, strObjectId,
	...
	public MapList getAllAssignee(Context context, String[] args) throws Exception {
		...
		String strRelName = PropertyUtil.getSchemaProperty(context, "relationship_TechnicalAssignee");
		...
		relBusObjPageList = dom.getRelatedObjects(context, strRelName, typePattern.getPattern(), objectSelects,
				relSelects, false, true, sLevel, "", "");
	...
	/**
	 * Change the Owner of Issues only if the context user is having change owner
	 * access privilege
	 *
	 * @param context - the eMatrix <code>matrix.db.Context</code> object
	 * @param args    - args contains a Map with the following entries: reqMap - the
	 *                request Map reqTableMap - the table map of Issue emxTableRowId
	 *                - The table row Id of Owner
	 * @throws Exception if the operation fails
	 * @since AEF 10.0.5.0
	 */
	@com.matrixone.apps.framework.ui.ProgramCallable
	public void changeOwner(Context context, String[] args) throws Exception {
		...
		for (int j = 0; j < arrBusIdList.length; j++) {
			...
			if ("true".equalsIgnoreCase((String) busMap.get(DomainConstants.SELECT_HAS_CHANGESOV_ACCESS))) {
				...
				String strAlert = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
						context.getLocale(), ALERT_NO_PREVELEGE);
	...
	/**
	 * Access is granted only if the context user is having role Issue Manager or
	 * Analyst or OCDX/OneClick VPLMCreator.
	 * 
	 * @param context <code>matrix.db.Context</code>
	 * @return boolean true only if the context user is having change owner access
	 *         privilege
	...
	private boolean hasAccess(Context context) throws MatrixException {
		...
		// boolean bIssueManager = strContextUser.isAssigned(context,
		// PropertyUtil.getSchemaProperty(context, Issue.SYMBOLIC_role_IssueManager));
		...
		// boolean bAnalyst = strContextUser.isAssigned(context,
		// PropertyUtil.getSchemaProperty(context, Issue.SYMBOLIC_role_Analyst));
		...
		boolean bVPLMContributor = strContextUser.isAssigned(context,
				PropertyUtil.getSchemaProperty(context, "role_VPLMExperimenter"));
		boolean bSecuredVPLMContributor = strContextUser.isAssigned(context,
				PropertyUtil.getSchemaProperty(context, "role_3DSRestrictedContributor"));
		boolean bVPLMAdmin = strContextUser.isAssigned(context,
				PropertyUtil.getSchemaProperty(context, "role_VPLMAdmin"));
		boolean bVPLMProjectOwner = strContextUser.isAssigned(context,
				PropertyUtil.getSchemaProperty(context, "role_VPLMProjectAdministrator"));
		boolean bSecuredVPLMProjectOwner = strContextUser.isAssigned(context,
				PropertyUtil.getSchemaProperty(context, "role_3DSRestrictedOwner"));

		if (bIssueManager || bAnalyst || bVPLMContributor || bVPLMAdmin || bVPLMProjectOwner || bSecuredVPLMContributor
				|| bSecuredVPLMProjectOwner) {
			bValidUser = true;
		}
	 ...
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries queryLimit -
	 *                The limit of the objects to be searched User Name - The user
	 *                name of the Person First Name - The First name of the Person
	 *                Last Name - The Last name of the Person ChangeOwner - mode
	 *                passed Company - The Company Name passed vaultOption - The
	 *                Vault option selected - All, Default, Selected VaultDisplay -
	 *                The displayed Vault name.
	 * @return MapList containing the Object ids of Persons matching the search
	 *         criteria
	 * @throws Exception if the operation fails
	 * @since AEF 10.0.5.0
	 */
	...
	public static MapList getPersons(Context context, String[] args) throws Exception {
		...
		if ((strFilteredExpression != null) && !("null".equalsIgnoreCase(strFilteredExpression))
		...
		if (!ChangeOwner.equals("true")) {
			mapList = DomainObject.findObjects(context, strType, strName, SYMB_WILD, SYMB_WILD, strVault,
					sbWhereExp.toString(), "", true, slSelect, sQueryLimit);
			return mapList;
		} else {
			...
			if (!iscloud) {
				...
				mapList = DomainObject.findObjects(context, strType, strName, SYMB_WILD, SYMB_WILD, strVault,
						sbWhereExp.toString(), true, slSelect);
				...
				while (Useriterator.hasNext()) {
					...
					if (mapList.contains(map)) {
						if (iCount <= sQueryLimit) {
							...
							String strWarning = MessageUtil.getMessage(context, null,
									"emxComponents.Warning.ObjectFindLimitReached", formatArgs, null,
									context.getLocale(), "emxComponentsStringResource");
	...
	/**
	 * The function to filter the object selection and apppend the default query in
	 * the where clause.
	 *
	 * @param context    - the eMatrix <code>Context</code> object
	 * @param programMap - with the following entries object Id - Object Id of the
	 *                   context object
	...
	protected static String getFilteredExpression(Context context, Map programMap) throws Exception {
		...
		if ((strCommand != null) && !strCommand.equals("") && !("null".equalsIgnoreCase(strCommand))) {
			...
			if ((strMode.equals(Search.ADD_EXISTING)) && (strObjectId != null) && (!strObjectId.equals(""))
	...
	public static String getDefaultVaults(Context context, String args[]) throws Exception {
		...
		try {
			...
			String strAll = i18nNowInstance.GetString("emxFrameworkStringResource", strLocale,
					"emxFramework.Preferences.AllVaults");
			String strDefault = i18nNowInstance.GetString("emxFrameworkStringResource", strLocale,
					"emxFramework.Preferences.UserDefaultVault");
			String strSelected = i18nNowInstance.GetString("emxFrameworkStringResource", strLocale,
					"emxFramework.Preferences.SelectedVaults");
			String strLocal = i18nNowInstance.GetString("emxFrameworkStringResource", strLocale,
			...
			radioOption.append(
					"&nbsp;&nbsp;<input id=\"displayVault\" type=\"text\" READONLY name=\"vaultsDisplay\" value =\""
							+ selDisplayVault + "\" id=\"\" size=\"20\" onFocus=\"this.blur();\">");
			radioOption.append(
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    - Holds the Hashmap containing following arguments queryLimit
	 *                Type Name
	...
	public static MapList getCompanies(Context context, String[] args) throws Exception {
		...
		try {
			...
			/*
			 * String strAttribOrgName = PropertyUtil.getSchemaProperty( context,
			 * Issue.SYMBOLIC_attribute_OrganizationName);
			 * 
			 * if (strName != null && (!strName.equals(SYMB_WILD)) && (!strName.equals(""))
			 * && !("null".equalsIgnoreCase(strName))) { sbWhereExp.append(SYMB_OPEN_PARAN);
			 * sbWhereExp.append(SYMB_ATTRIBUTE); sbWhereExp.append(SYMB_OPEN_BRACKET);
			 * sbWhereExp.append(strAttribOrgName); sbWhereExp.append(SYMB_CLOSE_BRACKET);
			 * sbWhereExp.append(SYMB_MATCH); sbWhereExp.append(SYMB_QUOTE);
			 * sbWhereExp.append(strName); sbWhereExp.append(SYMB_QUOTE);
			 * sbWhereExp.append(SYMB_CLOSE_PARAN); }
			 */
			...
			if ((strFilteredExpression != null) && !("null".equalsIgnoreCase(strFilteredExpression))
				...
				/*
				 * if (bStart) { sbWhereExp.append(SYMB_OPEN_PARAN); bStart = false; } else {
				 * sbWhereExp.append(SYMB_AND); }
				 */
	 ...
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - Holds the HashMap containing following arguments queryLimit
	 *                hdnType txtName txtDescription txtOwner txtState.
	...
	public static MapList findResolvedToReportedAgainst(Context context, String[] args) throws Exception {
		...
		try {
			...
			if (strType.equalsIgnoreCase(TYPE_DOCUMENTS)) {
				...
				sbWhereExp.append(SYMB_OPEN_PARAN);
				sbWhereExp.append(
						"attribute[" + DomainConstants.ATTRIBUTE_IS_VERSION_OBJECT + "] ~~ \"*" + "false" + "*\"");
				sbWhereExp.append(SYMB_CLOSE_PARAN);
			}
			if (!bStart) {
				sbWhereExp.append(SYMB_CLOSE_PARAN);
			...
			mapList = DomainObject.findObjects(context, strType, strName, strRevision, strOwner, strVault,
					sbWhereExp.toString(), "", true, (StringList) slSelect, sQueryLimit);
		} catch (Exception excp) {
			throw excp;
		}
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    args contains a Map with the following entries objectList -
	 *                list of assignee objects
	 * @return Vector - containing email ids
	 * @throws Exception if the operation fails
	 * @since AEF 10.0.5.0
	 * @deprecated @since R211 This method is deprecated in R211, just need to add
	 *             the format = email in the column/ field no need to have program
	 *             html column/field
	 */
...
//                      End : Bug 346997 code modification
	 ...
	 * @throws Exception if the operation fails
	 * @since AEF 10.0.5.0
	 * @deprecated @since R211 Just use format = user for column or field instead of
	 *             having programHTMLOutput
	 */
...
//              End : Bug 346997 code modification
		...
		for (int i = 0; i < iNoOfObjects; i++) {
			...
			if (obj instanceof HashMap) {
				...
				arrObjId[i] = (String) ((Hashtable) relBusObjPageList.get(i)).get(DomainConstants.SELECT_ID);
				arrRelId[i] = (String) ((Hashtable) relBusObjPageList.get(i))
		...
		for (int i = 0; i < iNoOfObjects; i++) {
			...
			// Begin of modify by Infosys for bug 297901, 04/19/2005
			if (((strPortalMode != null) && !("".equals(strPortalMode)) && !("null".equals(strPortalMode))
					&& (strPortalMode.equals("true")))
					|| ((strLaunchMode != null) && !("".equals(strLaunchMode)) && !("null".equals(strLaunchMode))
							&& (strLaunchMode.equals("true")))) {
				// Constructing the HREF
				strFullName = "<img src = \"images/iconPerson16.png\"><b><A HREF=\"JavaScript:emxTableColumnLinkClick('emxTree.jsp?mode=insert&name="
						+ strLastName + strFirstName + "&treeMenu=type_Person&emxSuiteDirectory="// Modified by Infosys
																									// for Bug # 304580
																									// Date 05/18/2005
						+ strSuiteDir + "&relId=" + strRelId + "&parentOID=" + strParentObjectId + "&jsTreeID="
						+ strJsTreeID + "&objectId=" + strObjId
						+ "', 'null', 'null', 'false', 'popup')\" class=\"object\">" + strLastName + ", " + strFirstName
						+ "</A>";
			}
//                     Begin : Bug 346997 code modification
			else if (!isExporting) {
//                     End : Bug 346997 code modification
				...
				strFullName =
						...
						"<img src = \"images/iconPerson16.png\"><b><A HREF=\"JavaScript:emxTableColumnLinkClick('emxTree.jsp?mode=insert&name="
								+ strLastName + strFirstName + "&treeMenu=type_Person&emxSuiteDirectory="// Modified by
																											// Infosys
																											// for Bug #
																											// 304580
																											// Date
								...
								+ strSuiteDir + "&relId=" + strRelId + "&parentOID=" + strParentObjectId + "&jsTreeID="
								+ strJsTreeID + "&objectId=" + strObjId
								+ "', 'null', 'null', 'false', 'content')\" class=\"object\">" + strLastName + ", "
								+ strFirstName + "</A>";
			}
//                     Begin : Bug 346997 code modification
			else {
...
//                     End : Bug 346997 code modification
	...
	/**
	 * Method call to update the Reporting Organization of the Issue as chosen by
	 * the user in the form page.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries: paramMap -
	 *                the Map of the parameters New Value - The new Value of
	 *                Organization Name
	 * @return int - Returns 0 in case of Check trigger is success and 1 in case of
	 *         failure
	 * @throws Exception if the operation fails
	 * @since AEF 10.0.5.0
	 */
	public int updateCompany(Context context, String[] args) throws Exception {
		...
		if (!strCurrentState.equals(strCreate)) {
			...
			// The new object id of the Organization that has to be used to connect with the
			// issue in context
			...
			String strRelReportingOrganization = PropertyUtil.getSchemaProperty(context,
	 ...
	 * @since AEF 10.0.5.0
	 */
	protected void updateConnection(Context context, String parentObjectId, String strRelationshipName,
			String strNewObjectId, boolean bIsFrom) throws Exception {
		...
		// Relationship id of the previous Organization and Issue is fetched for
		// disconnecting it.
		...
		// Function loop to avoid the connection from being established if no objects is
		// being passed.
	...
	public String getCompanyOrganizationChooserHTML(Context context, String[] args) throws Exception {
		...
		try {
			...
			strTypeChooser.append("<img src=\"images/iconCompany16.png\" border=\"0\" alt=\"Company\"></img><b>"
	...
	public String getDefaultCompanyNameHTML(Context context, String[] args) throws Exception {
		...
		strDefaultCompanyName.append("<input type=\"text\" READONLY name=\"OrganizationDisplay\" id=\"\" value=\""
				+ XSSUtil.encodeForHTMLAttribute(context, strUserCompanyName) + "\"");

		strDefaultCompanyName.append(" maxlength=\"\" size=\"\"><input type=\"hidden\" name=\"Organization\"  value=\""
				+ XSSUtil.encodeForHTMLAttribute(context, strUserCompanyId) + "\">");
		strDefaultCompanyName.append("<input type=\"hidden\" name=\"OrganizationOID\" value=\""
				+ XSSUtil.encodeForHTMLAttribute(context, strUserCompanyId) + "\">");
	 ...
	 *                queryLimit Type Name Description Priority OriginatorDisplay
	 *                IssueCategoryClassificationOID OwnerDisplay Organization
	 *                State.
	 * @return MapList containing the Object ids matching the search criteria
	 * @throws Exception if the operation fails
	 * @since AEF 10-0-5-0
	...
	public static MapList getFindIssues(Context context, String[] args) throws Exception {
		...
		if (strIssueCategoryClassification == null || strIssueCategoryClassification.equals("")
		...
		if (!strIssueCategoryClassification.equals(SYMB_WILD)) {
			...
			strClassification = strIssueCategoryClassification.substring(iIndex + 1,
		...
		String strAttribIssueClassification = PropertyUtil.getSchemaProperty(context,
				Issue.SYMBOLIC_attribute_IssueClassification);
		String strAttribPriority = PropertyUtil.getSchemaProperty(context, Issue.SYMBOLIC_attribute_Priority);
		String strRelReportingOrg = PropertyUtil.getSchemaProperty(context,
				Issue.SYMBOLIC_relationship_ReportingOrganization);
		...
		if (strCategory != null && (!strCategory.equals(SYMB_WILD)) && (!strCategory.equals(""))
		...
		if (strClassification != null && (!strClassification.equals(SYMB_WILD)) && (!strClassification.equals(""))
		...
		if ((strOrganization != null) && (!strOrganization.equals(SYMB_WILD)) && (!strOrganization.equals(""))
		...
		MapList mapList = null;
		mapList = DomainObject.findObjects(context, strType, strName, SYMB_WILD, strOwner, strVault,
	...
	/**
	 * A trigger method to notify Originator of the Issue wile promoting from create
	 * to assign.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	...
	/**
	 * A trigger method to set the actual start date when the issue is promoted from
	 * Assign to Active state.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - holds the following argument 0 - string containg the Object
	 *                Id
	...
	public int setActualStartDate(Context context, String[] args) throws Exception {
		...
		try {
			...
			String strAttribActStartDate = PropertyUtil.getSchemaProperty(context,
	...
	/**
	 * A trigger method to reset the Actual Start Date when the issue is demoted
	 * from Active to Assign state.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    holds the following arguments: 0 - string containing the
	 *                Object Id
	 * @return - Integer value 0 indicating success
	 * @throws Exception if the operation fails
	 * @since AEF 10.0.5.0
	 ...
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    holds the following arguments: 0 - string containing the
	 *                Object Id 1 - string containing the Assignee Id
	 * @return - Integer value 0 indicating success
	 * @throws Exception if the operation fails
	 * @since AEF 10.0.5.0
	 */
	public int notifyAssignees(Context context, String[] args)
		...
		try {
			...
			if (mailNotificationEnabled) {
				...
				MailUtil mail = new MailUtil();
				String subject = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
						context.getLocale(), MESSAGE_ASSIGNMENT_SUBJECT);
				String message = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
						context.getLocale(), MESSAGE_ASSIGNMENT_BODY);
	...
	/**
	 * A trigger method to set the Actual End Date when the issue is promoted to
	 * Close state.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    holds the following arguments 0 - String containing the Object
	 *                Id
	 * @return - Integer value 0 indicating success
	 * @throws Exception if the operation fails
	 * @since AEF 10.0.5.0
	...
	public int setActualEndDate(Context context, String[] args) throws Exception {
		try {
			...
			String strAttribActEndDate = PropertyUtil.getSchemaProperty(context,
	...
	/**
	 * A trigger method to reset the Actual End Date and to notify Originator of the
	 * Issue wile demoting from Close state.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    holds the following arguments: 0 - String containing the
	 *                Object Id
	...
	/**
	 * A trigger method check whether the Estimated Start Date and Estimated End
	 * Date are filled or not.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    holds the following arguments: 0 - String containing the
	 *                Object Id
	...
	public int checkEstimatedDates(Context context, String[] args) throws Exception {
		...
		try {
			...
			String strAttribEstStartDate = PropertyUtil.getSchemaProperty(context,
					Issue.SYMBOLIC_attribute_EstimatedStartDate);
			String strEstimatedStartDate = getAttributeValue(context, strAttribEstStartDate);

			String strAttribEstEndDate = PropertyUtil.getSchemaProperty(context,
			...
			if (UIUtil.isNullOrEmpty(strEstimatedStartDate) && UIUtil.isNullOrEmpty(strEstimatedEndDate)) {
				String strDateError = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
						strLocale, EST_DATE_ERROR);
				emxContextUtil_mxJPO.mqlNotice(context, strDateError);
				iFlag = 1;
			} else if (UIUtil.isNullOrEmpty(strEstimatedStartDate)) {
				String strDateError = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
						strLocale, EST_START_DATE_ERROR);
				emxContextUtil_mxJPO.mqlNotice(context, strDateError);
				iFlag = 1;
			} else if (UIUtil.isNullOrEmpty(strEstimatedEndDate)) {
				String strDateError = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
						strLocale, EST_END_DATE_ERROR);
				emxContextUtil_mxJPO.mqlNotice(context, strDateError);
				iFlag = 1;
			}
	...
	/**
	 * A trigger method check whether the attributes 'Action Taken', 'Resolution
	 * Statement' and Resolution Date are filled or not before promoting from Review
	...
	public int checkCloseAttributes(Context context, String[] args) throws Exception {
		...
		try {
			...
			String strAttribResolutionStatement = PropertyUtil.getSchemaProperty(context,
			...
			String strLanguage = context.getSession().getLanguage();
			Locale strLocale = context.getLocale();
			if (UIUtil.isNullOrEmpty(strActionTaken) && UIUtil.isNullOrEmpty(strResolutionStatement)) {
				String strCloseError = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
						strLocale, CLOSE_ERROR);
				emxContextUtil_mxJPO.mqlNotice(context, strCloseError);
				iFlag = 1;
			} else if (UIUtil.isNullOrEmpty(strActionTaken)) {
				String strCloseError = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
						strLocale, CLOSE_ACTION_TAKEN_ERROR);
				emxContextUtil_mxJPO.mqlNotice(context, strCloseError);
				iFlag = 1;
			} else if (UIUtil.isNullOrEmpty(strResolutionStatement)) {
				String strCloseError = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
						strLocale, CLOSE_RESL_STATEMENT_ERROR);
				emxContextUtil_mxJPO.mqlNotice(context, strCloseError);
				iFlag = 1;
			}
			/* check for Resolution Date while closing */
				...
				String strCloseError = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
						strLocale, CLOSE_RESL_DATE_ERROR);
				emxContextUtil_mxJPO.mqlNotice(context, strCloseError);
				iFlag = 1;
				/* check for Resolution Date while closing */
			}
	 ...
	 *                IssueActionTaken - The Value to be filled in attribute 'Action
	 *                Taken' IssueResolutionStatement - The Value to be filled in
	 *                attribute 'Resolution Statement' objectId - The Object Id of
	 *                the Context
	...
	public String closeIssue(Context context, String[] args) throws FrameworkException {
		try {
			...
			/*
			 * The exception ex is thrown back as FrameworkException. This can be modified
			 * to handle some specific message from back end and do internationalization.
			 */
	...
	/**
	 * Method to calculate the Slip Days based on Actual End Dates or Estimated End
	 * Dates.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - holds the following arguments: objectList - Map containing
	 *                Issue Ids paramList - Map containing parameters
	...
	/**
	 * Method to check for display for the Date field with Date Picker based on
	 * Lifecycle States.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries objectId -
	 *                Object Id of the Context object
	...
	/**
	 * Method to check for display for the Date field without Date Picker (non
	 * editable) based on Lifecycle States.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries objectId -
	 *                The object Id of Context object
	 * @return - boolean (true or false)
	 * @throws Exception if the operation fails
	 * @since AEF 10.0.5.0
	 */
	...
	/**
	 * Method to check for displaying the chooser for a field in the Webform based
	 * on Lifecycle States.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries objectId -
	...
	/**
	 * Method to check for not displaying the chooser for a field in the Webform
	 * based on Lifecycle States.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries objectId -
	 *                The object Id of Context object
	 ...
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries objectId -
	 *                The object Id of Context object settings - hashmap containing
	 *                the settings
	 * @return - boolean (true or false)
	 * @throws Exception if the operation fails
	 * @since AEF 10.0.5.0
	...
	/**
	 * Trigger Method to check whether the state of the Issue object is 'Create' and
	 * also the accesses for relationship Assigned Issue.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains the Ids of Context Issue Object, Selected
	 *                Person object and Relation
	...
	public int checkPerson(Context context, String[] args) throws Exception {
		...
		try {
			...
			if (strCurrentState.equals(strActualStateName)) {
				...
				String strAssignedStateError = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
						strLocale, CHECK_STATE);
				...
				boolean bIssueManager = false;
				if (!iscloud) {
					String strRoleIssueManager = PropertyUtil.getSchemaProperty(context,
...
// Modified condition for checking analyst role //
				...
				if (iFlag == 1) {
					...
					mlAssigneeList = domCategoryState.getRelatedObjects(context, strReln, strType, slObjSelects, null,
							true, false, (short) 1, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
				...
				if (iFlag == 1) {
					...
					String strAccessError = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
							strLocale, CHECK_ACCESS);
		...
		for (int i = 0; i < iNoOfObjects; i++) {
			...
			if (strMode != null && strMode.equals(Search.GLOBAL_SEARCH)) {
				strFullName = "<A HREF=\"JavaScript:emxTableColumnLinkClick('emxTree.jsp?mode=insert&emxSuiteDirectory="
						+ strSuiteDir + "&defaultsortname=" + attributeList.getElement(i).getSelectData(strAttrb2)
						+ "&parentOID=" + strParentObjectId + "&jsTreeID=" + strJsTreeID + "&objectId=" + strObjId
						+ "', 'null', 'null', 'false', 'content')\" class=\"object\">"
						+ attributeList.getElement(i).getSelectData(strAttrb2) + "</A>";
			} else {
	...
	/**
	 * Method to check whether the person is 'Issue Manager' or 'Originator' or
	 * 'Assignee' for Create New Documents Link.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries objectId -
	 *                The object Id of Context object
		...
		try {
			...
			if (!bFlag) {
				...
				if (!bFlag) {
					...
					// Running the Query to get Assignees
					mlAssigneeList = domIssueObj.getRelatedObjects(context, strReln, strType, slObjSelects, null, true,
							false, (short) 1, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
	...
	/**
	 * A trigger method to notify Originator of the Issue wile promoting from reveiw
	 * to closed state.
	 *
	 * @param context - the eMatrix <code>Context</code> object
		...
		try {
			...
			if (mailNotificationEnabled) {
				...
				if ("owner".equals(kindOfOwner)) {
					...
					// Check if Originator is himself a owner. If yes then the message should be
					// blank. Otherwise message should contain all details
					message = strOwner.equals(strOriginator) ? ""
							: Issue.replaceMacroWithValues(context, strObjectId, message);
	 ...
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    contains a Map with the following entries: strObjectId -
	...
	public static Object getIssueCategory(Context context, String[] args) throws Exception {
		...
		/*
		 * check whether there is an entry in the property file if there is an entry
		 * return the internationalised value else return the value as it is
		 */
		...
		StringBuffer sbCategoryStr = new StringBuffer("emxComponents.Common.")
				.append(strTempIssueCategory.replace(' ', '_'));
		String strMXIssueCategory = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
				context.getLocale(), sbCategoryStr.toString());
	...
	public static Object getIssueClassification(Context context, String[] args) throws Exception {
		...
		String strSelect = "attribute["
				+ PropertyUtil.getSchemaProperty(context, SYMBOLIC_attribute_IssueClassification) + "]";
		DomainObject domObj = newInstance(context, strObjectId);
		String strTempIssueClassification = domObj.getInfo(context, strSelect);
		StringBuffer sbClassificationStr = new StringBuffer("emxComponents.Common.")
				.append(strTempIssueClassification.replace(' ', '_'));
		String strMXIssueClassification = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
				context.getLocale(), sbClassificationStr.toString());
	...
	/**
	 * Method to display the Classified Item column or not if called from Rollup
	 * Issues.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries objectId -
	...
	/**
	 * Method call to get all the Issues under the context Library Hierarchy Tree
	 * Level from the data base.
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    - holds HashMap programMap which has Object Id to be used by
	 *                this method
	 * @return MapList containing the id of all Issue objects
	 * @throws Exception if the operation fails
	 * @since Common 11.0
	...
	public MapList getRollupIssues(Context context, String[] args) throws Exception {
		...
		ExpansionIterator expItr = domObj.getExpansionIterator(context, sbRelNames.toString(), "*", objectSelects,
				new StringList(), false, true, (short) 1, "", "", (short) 0, true, false, (short) 1000, true);
	...
	/**
	 * Method call to get Issues connected to Reference Documents from the data
	 * base.
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    - holds HashMap programMap which has Object Id to be used by
	...
	public MapList getRelatedIssues(Context context, String[] args) throws Exception {
		...
		// retrieving Issue List from Context
		relIssueList = (MapList) domObj.getRelatedObjects(context, strRelationIssue, strType, objectSelects, null, true,
				false, (short) 1, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
	 ...
	 *                HashMap containing the following keys, "objectId", "old
	 *                RDOIds", "New OID", "Old RDO Rel Ids".
	...
	public String getResolutionDate(Context context, String[] args) throws Exception {
		...
		String fieldValueDisplay = (String) map.get(
				"attribute[" + PropertyUtil.getSchemaProperty(context, Issue.SYMBOLIC_attribute_ResolutionDate) + "]");

		// Added for the Bug 345286 1

		if (fieldValueDisplay != null && fieldValueDisplay.length() > 0) {
			formattedDisplayDateValue = eMatrixDateFormat.getFormattedDisplayDateTime(context, fieldValueDisplay, false,
					iDateFormat, iClientTimeOffset, locale1);
		}
		...
		if (strOriginated != null && strOriginated.length() > 0) {
			originatedDate_formattedvalue = eMatrixDateFormat.getFormattedDisplayDateTime(context, strOriginated, false,
					iDateFormat, iClientTimeOffset, locale1);
		}
		...
		// TP2: Added check for mode == "" and checking with OR condition instead of AND
		// for bug # IR-023865V6R2011 Start

		// if ((mode == "" || mode == null || mode.equalsIgnoreCase("view"))

		// || (formattedDisplayDateValue != null && formattedDisplayDateValue.length() >
		// 0)) {
		...
		// Modified for IR-072793V6R2012 - reversed the strings in equalsIgnoreCase
			...
			outStr.append(
					"\" value='" + XSSUtil.encodeForHTMLAttribute(context, formattedDisplayDateValue) + "'\"\" id=\"");
			...
			// <a href="javascript:;" onclick="javascript:showCalendar('editDataForm',
			// 'ResolutionDate', '2/17/2009 12:00:00 PM', '',
			// saveFieldObjByName('EstimatedStartDate')); return false;"
			// name="ResolutionDate_date" >
			// <img src="../common/images/iconSmallCalendar.gif" alt="Date Picker"
			// border="0">
			// </a>
			...
			outStr.append("saveFieldObjByName('").append(fieldName).append("')); return false;\" name=\"")
					.append(fieldName).append("_date\">");
			...
			try {
				i18strClear = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource",
						new Locale(languageStr), "emxFramework.FormComponent.Clear");
			} catch (Exception e) {

			}
			if (showClearButton != null && showClearButton.equalsIgnoreCase("true")) {
				fieldClearURL = "<a href=\"javascript:basicClear('" + fieldName + "')\">" + i18strClear + "</a>";
			}
			outStr.append("&nbsp;");
			outStr.append(fieldClearURL);
			outStr.append("<input type=hidden name='OriginatedDate' value='" + map.get("originated") + "'>");
			outStr.append("<input type=hidden name='originatedDate_formattedvalue' value='"
					+ XSSUtil.encodeForHTMLAttribute(context, originatedDate_formattedvalue) + "'>");// Added for
																										// IR-053232V6R2011x
			outStr.append("<input type=hidden name='ServerCurrentDate' value='"
	...
	/**
	 * Connects the Design Responsibility Organization to the Issue Object. This
	 * method is called for Issue objects as update function in their respective
	 * Edit pages.
	 * 
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds a HashMap containing the following entries: paramMap - a
		...
		try {
			...
			if (!strOldRDOIds.equals(strNewOrganizationId)) {
				...
				if (newOIDIndex == -1 && !"".equals(strNewOrganizationId)) {
					...
					DomainRelationship.connect(context, this, DomainConstants.RELATIONSHIP_DESIGN_RESPONSIBILITY,
	...
	/**
	 * Gets the Organizations connected to Issue Objects with the relationship
	 * Design Responsibility. This is used in the display of properties page of
	 * these objects, normally called from Webform.
	 * 
	 * @param context the eMatrix <code>Context</code> object.
	 * @param args    holds the following input arguments: 0 - String containing the
	 *                object id.
	 * @return String of Organizations connected to the Object with the relationship
	 *         Design Responsibility in HTML format.
	 * @throws Exception if the operation fails.
	 * @since Common 11.0
	 */
	...
	public String getResponsibleOrganization(Context context, String[] args) throws Exception {
		...
		organizationList = getRelatedObjects(context, DomainConstants.RELATIONSHIP_DESIGN_RESPONSIBILITY, "*",
				ObjectSelectsList, relSelectList, true, true, (short) 1, DomainConstants.EMPTY_STRING,
				DomainConstants.EMPTY_STRING);
		ContextUtil.popContext(context);
			...
			while (mapItr.hasNext()) {
				...
				if ("edit".equalsIgnoreCase(strMode)) {
					...
					if (strAccess.equalsIgnoreCase("TRUE")) {
						...
						strBufRDONamesForView
								.append("<a href=\"javascript:showModalDialog(\'" + URLToShow + "\',700,600,false)\">");
						strBufRDONamesForView.append(mapOrg.get(DomainConstants.SELECT_ATTRIBUTE_TITLE) + "</a>");
						strBufRDONamesForView.append(",");
					} else {
						strBufRDONamesForView.append(mapOrg.get(DomainConstants.SELECT_ATTRIBUTE_TITLE));
						strBufRDONamesForView.append(",");
					}
					if (null != reportFormat && !reportFormat.equals("null") && (reportFormat.length() > 0)) {
						strBufRDONamesForExport.append(mapOrg.get(DomainConstants.SELECT_ATTRIBUTE_TITLE));
					}
					if (null != printFormat && !printFormat.equals("null") && (printFormat.length() > 0)) {
						strBufRDONamesForPrint.append(mapOrg.get(DomainConstants.SELECT_ATTRIBUTE_TITLE));
					}
				}
			}
		}
		...
		if ("edit".equalsIgnoreCase(strMode)) {
			...
			returnString.append("<input type=\"text\" readonly=\"readonly\"  name=\"RDODisplay\"  value=\""
					+ XSSUtil.encodeForHTMLAttribute(context, strBufRDOTitle.toString()) + "\">");
			returnString.append("<input type=\"hidden\" name=\"RDO\" value=\""
					+ XSSUtil.encodeForHTMLAttribute(context, strBufRDONames.toString()) + "\">");
			returnString.append("<input type=\"hidden\" name=\"RDOOID\" value=\""
					+ XSSUtil.encodeForHTMLAttribute(context, strBufRDOIds.toString()) + "\">");
			returnString.append("<input type=\"hidden\" name=\"RDORELID\" value=\""
					+ XSSUtil.encodeForHTMLAttribute(context, strBufRELIds.toString()) + "\">");
			returnString.append("<input type=\"hidden\" name=\"OLDRDOID\" value=\""
					+ XSSUtil.encodeForHTMLAttribute(context, strBufRDOIds.toString()) + "\">");
			returnString.append("<input type=\"button\" name=\"btnRDO\" value='...'  "
			...
			if (!strDisable.equals("disabled")) {
				returnString.append("&nbsp;&nbsp;<a href=\"JavaScript:basicClear('RDO')\">"
						+ ComponentsUtil.i18nStringNow("emxComponents.Common.Clear", languageStr) + "</a>");
			}
		} else {
	...
	/**
	 * sets the Co Owners with the new list, normally called from Webform while
	 * editing.
	 * 
	 * @param context the eMatrix <code>Context</code> object.
	 * @param args    holds a HashMap containing the following entries: programMap -
	 *                a HashMap containing the following keys, "requestMap"
	 *                "paramMap".
	 * @throws Exception if operation fails
	 * @since Common 11.0
	 */
	public void setCoOwners(Context context, String[] args) throws Exception {
		...
		if (!(strOldCoOwners.equals(strNewCoOwners))) {
			...
			/*
			 * setAttributeValue(context, ATTRIBUTE_CO_OWNER, strNewCoOwners);
			 * issue.updateIssueCoOwnerOwnership(context, strIssueID,
			 * oldCoOwners,newCoOwners);
			 */
			// ISU 23x - new API/SCHEMA
			// add new users
			...
			MailUtil.sendNotification(context, addedCoOwners, null, null,
					FrameworkStringResource.MultipleOwnerManager_AddCoOwnerSubject, subjectKeys, subjectValues,
					FrameworkStringResource.MultipleOwnerManager_AddCoOwnerMessage, messageKeys, messageValues,
					objectIdList, Company.getCompanyForKey(context, Person.getCompanyKey(context)).getName());
		}
	}

	/**
	 * Gets the Coowners This is used in the display of properties page of the
	 * object, normally called from Webform.
	 * 
	 * @param context the eMatrix <code>Context</code> object.
	 * @param args    holds the following input arguments: 0 - String containing the
	 *                object id.
	...
	/**
	 * Method to display the Create Issue command. If the type is Classified Part or
	 * Document which connected to Classified Part.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries objectId -
	 *                Object Id of the Context object
	...
	public boolean showCreateIssue(Context context, String[] args) throws Exception {
		...
		if (loggedInRole.contains(roleOwner) || loggedInRole.contains(roleSecuredOwner)
		...
		if (!(strObjectId == null || "".equals(strObjectId))) {
			...
			if (strParentType.equals(TYPE_DOCUMENTS)) {
				...
				String strRelClassPart = dom.getInfo(context,
						"to[" + DomainConstants.RELATIONSHIP_CLASSIFIED_ITEM + "].id");
	...
	/**
	 * Method to display the Issues command in the Tree Category. In Library or
	 * Class Level if the Logged in user is Librarian or in Parts or Documents, if
	 * the type is Classified Part or Document which connected to Classified Part.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries objectId -
	 *                Object Id of the Context object
	...
	public boolean showRollupIssue(Context context, String[] args) throws Exception {
		...
		if (!(strObjectId == null || "".equals(strObjectId))) {
			...
			if (strParentType.equals(TYPE_LIBRARIES) || strParentType.equals(TYPE_CLASSIFICATION)) {
				...
				String role_VPLMProjectAdministrator = PropertyUtil.getSchemaProperty(context,
						"role_VPLMProjectAdministrator");
				String role_SecuredVPLMProjectLeader = PropertyUtil.getSchemaProperty(context,
						"role_3DSRestrictedLeader");
				String role_SecuredVPLMProjectAdministrator = PropertyUtil.getSchemaProperty(context,
						"role_3DSRestrictedOwner");

				// To Check whether the user is assigned the role
				// Librarian,VPLMProjectLeader,VPLMProjectAdministrator...
				return bLibrarian || pUser.isAssigned(context, role_VPLMProjectLeader)
				...
				if (strParentType.equals(TYPE_DOCUMENTS)) {
					 ...
					 * DomainConstants.RELATIONSHIP_CLASSIFIED_ITEM + "].id"); return
					 * (!(strRelClassPart == null || "".equals(strRelClassPart) ||
	...
	/**
	 * Shows the Reported Against as Type Name Revision. This is used to display in
	 * Rollup Issues
	 * 
	 * @param context the eMatrix <code>Context</code> object.
	 * @param args    holds the following input arguments: 0 - String containing the
	...
	public Vector getReportedAgainst(Context context, String[] args) throws Exception {
		...
		for (int i = 0; i < relBusObjPageList.size(); i++) {
			...
			if (!(strObjectId == null || "".equals(strObjectId))) {
				...
				/*
				 * DomainObject dom = new DomainObject(strObjectId); String strRelationIssue =
				 * PropertyUtil.getSchemaProperty(context, Issue.SYMBOLIC_relationship_Issue);
				 * StringList slRepAgainstIds = dom.getInfoList(context,"from[" +
				 * strRelationIssue + "].to.id");
				 */
				// 23x ISU new schema ---
				MapList relBusObjPageList2 = new Issue().getAllAffectedItems(context, strObjectId,
						new StringList(DomainConstants.SELECT_ID));
				...
				while (repAgainstIdItr.hasNext()) {
					...
					if ((paramList.get("reportFormat")) != null) {
						...
						strShowLink += "<a href=\"javascript:showModalDialog(\'" + URLToShow + "\',700,600,false)\">"
								+ XSSUtil.encodeForXML(context, strRepAgainstType) + " "
								+ XSSUtil.encodeForXML(context, strRepAgainstName) + " "
								+ XSSUtil.encodeForXML(context, strRepAgainstRev) + "</a>";
					}
				}
			} else {
				strShowLink = "";
	...
	public String getNotificationHTML(Context context, String[] args) throws Exception {
		...
		String message = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR, context.getLocale(),
				MESSAGE_MODIFY_BODY);
	 ...
	 * @param args    - Holds the parameters passed from the calling method
	 *                queryLimit Type Name Description Owner State latestOnly
	 *                TypeDocument Revision vaultOption VaultDisplay.
	...
	public static Object getVCDocuments(Context context, String[] args) throws Exception {
		...
		mapList = DomainObject.findObjects(context, strType, strName, "*", strOwner, "*", sbWhereExp.toString(), "",
				true, select, sQueryLimit);
		...
		for (int i = 0; i < mapList.size(); i++) {
			...
			if ((DesignSyncFolder != null && (!DesignSyncFolder.equals("")))
					&& (DesignSyncModule != null && (!DesignSyncModule.equals("")))) {
				...
				if (type.equals(PropertyUtil.getSchemaProperty(context, "type_mxsysDSFAHolder"))) {
					...
					if ((storePath.equals("/") || storePath.equals(""))) {
						...
						String mxsysRevision = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump", id,
								"revision");
	...
	/**
	 * added for the Bug 360573 issueDeleteCheck - gets the list of Discussion
	 * objects connected to the context Issue
	 * 
	 * @param context the eMatrix <code>Context</code> object
	 * @param args    holds the following input arguments: 0 - Route Object Id
	 * @returns int
	 * @throws Exception if the operation fails
	...
	public int issueDiscussionDeleteCheck(Context context, String[] args) throws Exception {
		try {
			...
			Pattern relPattern = new Pattern(
					PropertyUtil.getSchemaProperty(context, DomainSymbolicConstants.SYMBOLIC_relationship_Thread));
			relPattern.addPattern(
					PropertyUtil.getSchemaProperty(context, DomainSymbolicConstants.SYMBOLIC_relationship_Message));
			Pattern typePattern = new Pattern(
					PropertyUtil.getSchemaProperty(context, DomainSymbolicConstants.SYMBOLIC_type_Thread));
			typePattern
					.addPattern(PropertyUtil.getSchemaProperty(context, DomainSymbolicConstants.SYMBOLIC_type_Message));
			DomainObject issueObj = new DomainObject(args[0]);
			MapList list = issueObj.getRelatedObjects(context, relPattern.getPattern(), typePattern.getPattern(),
					objectSelects, null, false, true, (short) 0, null, null);
...
// IR-060833V6R2011x modification -Starts
	...
	public String getTypesForWorkspaceContent(Context context, String args[]) throws Exception {
		...
		try {
			...
			strTypeChooser.append("<input type=\"text\" READONLY name=\"TypeDisplay\" value =\""
					+ DomainConstants.TYPE_DOCUMENT + "\" id=\"\" size=\"20\" onFocus=\"this.blur();\">");
			strTypeChooser.append(
					"<input type=\"button\" name=\"TypeChooseButton\" value=\"...\" onclick=\"javascript:getTopWindow().showChooser('../common/emxTypeChooser.jsp?frameName=searchPane&formName=editDataForm&SelectType=singleselect&SelectAbstractTypes=true&InclusionList="
							+ strTypes
							+ "&observeHidden=false&ShowIcons=true&fieldNameActual=Type&fieldNameDisplay=TypeDisplay','400','400')\">");
			strTypeChooser.append("<input type=\"hidden\" name=\"Type\" value=\"");
...
// IR-060833V6R2011x modification -Ends
		...
		try {
			...
			if (!getInfo(context, SELECT_CURRENT).equals(strFinalState)) {
				throw new FrameworkException(
						ComponentsUtil.i18nStringNow("emxComponents.Alert.CloseIssueUnSuccessfull", language));
			}
	...
	public MapList issueCategoryChooser(Context context, String[] args) throws FrameworkException {
		...
		try {
			...
			// running the query
			mapBusIds = findObjects(context, strType, null, null, null, DomainConstants.QUERY_WILDCARD,
					strWhereExpression, false, busSelects);
	...
	public MapList issueCategoryExpand(Context context, String[] args) throws FrameworkException {
		...
		try {
			...
			classlist = domCategory.getRelatedObjects(context,
					PropertyUtil.getSchemaProperty(context, SYMBOLIC_relationship_IssueCategoryClassification), strType,
					busSelects, null, false, true, (short) 1, DomainConstants.EMPTY_STRING,
					DomainConstants.EMPTY_STRING);
	...
	/**
	 * Method returns the updated MapList containing status icon for status field in
	 * widgets
	 * 
	 * @param context
	 * @param args
	...
	public MapList getSlipDaysIconForWidgets(Context context, String[] args) throws Exception {
		...
		for (int i = 0; i < widgetDataMapList.size(); i++) {
			...
			String strEstimatedEndDate = (String) collMap
					.get("attribute[" + DomainConstants.ATTRIBUTE_ESTIMATED_END_DATE + "]");
			String strActualEndDate = (String) collMap
					.get("attribute[" + DomainConstants.ATTRIBUTE_ACTUAL_END_DATE + "]");
	 ...
	 * @since V6R2014x
	 */
	private String getStatusIconTag(Context context, String strEstimatedEndDate, String strActualEndDate,
			String strState, String baseURI) throws Exception {
		...
		String strTooltipRed = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
				context.getLocale(), ICON_TOOLTIP_RED);
		String strTooltipGreen = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
				context.getLocale(), ICON_TOOLTIP_GREEN);
		String strTooltipYellow = EnoviaResourceBundle.getProperty(context, RESOURCE_BUNDLE_COMPONENTS_STR,
				context.getLocale(), ICON_TOOLTIP_YELLOW);
		...
		if (!(strState.equalsIgnoreCase(strClose)) && !(strState.equalsIgnoreCase(""))) {
			...
			if (strEstimatedEndDate.equalsIgnoreCase(DomainConstants.EMPTY_STRING)
					|| strActualEndDate.equalsIgnoreCase(DomainConstants.EMPTY_STRING)) {
				strStatusIconTag = "&#160;";
			} else if (!(strEstimatedEndDate.equalsIgnoreCase(DomainConstants.EMPTY_STRING))
					&& !(strActualEndDate.equalsIgnoreCase(DomainConstants.EMPTY_STRING))) {
	...
	public MapList getIssuescockpitItems(Context context, String[] args) throws Exception {
		...
		sbWhere.append("] == TRUE").append(") && ");
		sbWhere.append(" (from[").append(PropertyUtil.getSchemaProperty(context, "relationship_TechnicalAssignee"));
		sbWhere.append("].to[Group Proxy].from[")
				.append(PropertyUtil.getSchemaProperty(context, "relationship_GroupMember")).append("].to == '")
				.append(context.getUser()).append("')) ||");
		...
		if (UIUtil.isNotNullAndNotEmpty(sFilterProblemType)) {
			busSelects.add(
					"attribute[" + PropertyUtil.getSchemaProperty(context, Issue.SYMBOLIC_attribute_ProblemType) + "]");
			sbWhere.append(" && (" + "attribute["
					+ PropertyUtil.getSchemaProperty(context, Issue.SYMBOLIC_attribute_ProblemType) + "]" + " == '")
					.append(sFilterProblemType).append("')");
		}
		if (UIUtil.isNotNullAndNotEmpty(sFilterPriority)) {
			busSelects.add(
					"attribute[" + PropertyUtil.getSchemaProperty(context, Issue.SYMBOLIC_attribute_Priority) + "]");
			sbWhere.append(" && (" + "attribute["
					+ PropertyUtil.getSchemaProperty(context, Issue.SYMBOLIC_attribute_Priority) + "]" + " == '")
					.append(sFilterPriority).append("')");
		}

		if (UIUtil.isNotNullAndNotEmpty(sFilterGlobal)) {
			if ("In Work".equals(sFilterGlobal)) {
				sbWhere.append(
						" && ((" + DomainConstants.SELECT_CURRENT + " == " + DomainConstants.STATE_ISSUE_ASSIGN + ")");
				sbWhere.append(
						" || (" + DomainConstants.SELECT_CURRENT + " == " + DomainConstants.STATE_ISSUE_ACTIVE + "))");
			} else if ("Not Complete".equals(sFilterGlobal)) {
				sbWhere.append(
						" &&(" + DomainConstants.SELECT_CURRENT + " != " + DomainConstants.STATE_ISSUE_CLOSE + ")");
		...
		try {
			...
			if (isRDFQuery && UINavigatorUtil.isCloud(context)) {
				...
				while (strToken.hasMoreTokens()) {
					...
					String symbolicName = UIUtil.isNullOrEmpty(PropertyUtil.getSchemaProperty(context, temp[0]))
							? temp[0]
							: PropertyUtil.getSchemaProperty(context, temp[0]);
			...
			boolean to = false;
			/*
			 * if("true".equalsIgnoreCase((String)paramMap.get("isTo"))) { from =false; to
			 * =true; }else { from =true; to =false; }
			 */
			...
			if (UIUtil.isNotNullAndNotEmpty(rel) && (rel.equalsIgnoreCase(resolvedToRel)
					|| rel.equalsIgnoreCase(issueRel) || rel.equalsIgnoreCase(referenceDocumentRel))) {
				rel = issueRel + "," + referenceDocumentRel + "," + resolvedToRel;
			}

			MapList mlist = domainObject.getRelatedObjects(context, rel, strtype.toString(), selectList, null, true,
	...
	public static StringList excludeIssueReportedAgainst(Context context, String[] args) throws Exception {
		...
		try {
			MapList relBusObjPageList = new Issue().getAllAffectedItems(context, strParentId,
					new StringList(DomainConstants.SELECT_ID));
	...
	@com.matrixone.apps.framework.ui.ProgramCallable
	public String getCompanyName(Context context, String[] args) throws Exception {
		String strBaseType = PropertyUtil.getSchemaProperty(context, Issue.SYMBOLIC_type_Issue);
	...
	/**
	 * This method is field program to retrieve the Context object as Reported
	 * Against
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    holds request Map
	 ...
	 */
	@com.matrixone.apps.framework.ui.ProgramCallable
	public Object getContextObjAsReportedAgainst(Context context, String[] args) throws Exception {
		...
		sbReturnString
				.append("<input type=\"text\" name=\"Reported Against1Display\" id=\"\" readonly=\"true\" value=\""
						+ XSSUtil.encodeForXML(context, strReturnContextObjList.toString())
						+ "\" maxlength=\"\" size=\"\"></input>");
		sbReturnString.append("<input type=\"hidden\" name=\"Reported Against1\" value=\""
				+ XSSUtil.encodeForXML(context, strReturnContextObjList.toString()) + "\"></input>");
		sbReturnString.append("<input type=\"hidden\" name=\"Reported Against1OID\" value=\""
				+ XSSUtil.encodeForXML(context, strContextObjectIdList.toString()) + "\"></input>");
		return sbReturnString.toString();
	}

	/**
	 * This is access program to check under which context Issue creation is
	 * triggered
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    holds request Map
	 * @return boolean - true/false - true if Issue is created under an Issue using
	 *         'Resolved By', false otherwise
	 * @throws Exception if the operation fails
	 * @since AEF 418.HF5
	 */
	@com.matrixone.apps.framework.ui.ProgramCallable
	public static boolean checkIssueCreateContext(Context context, String args[]) throws Exception {
		...
		if ((UIUtil.isNotNullAndNotEmpty(strContextObjectId) || UIUtil.isNotNullAndNotEmpty(emxTableRowIds))
	 ...
	 */
	@com.matrixone.apps.framework.ui.ProgramCallable
	public void updateReportingOrganization(Context context, String[] args) throws Exception {
		...
		if (UIUtil.isNotNullAndNotEmpty(strCompanyId)) {
			String strRelReportingOrganization = PropertyUtil.getSchemaProperty(context,
					Issue.SYMBOLIC_relationship_ReportingOrganization);
			DomainRelationship.connect(context, new DomainObject(strCompanyId), strRelReportingOrganization,
					issueDomObj);
		}
	}
	 ...
	 */
	@com.matrixone.apps.framework.ui.ProgramCallable
	public void updateReportedAgainst(Context context, String[] args) throws Exception {
		HashMap progMap = (HashMap) JPO.unpackArgs(args);
		HashMap paramMap = (HashMap) progMap.get("paramMap");
		...
		if (UIUtil.isNullOrEmpty(strReportedObjs)) {
			String message = EnoviaResourceBundle.getProperty(context, "emxFrameworkStringResource",
					context.getLocale(), "emxFramework.FormComponent.MustEnterAValidValueFor") + " "
		...
		if (fromObjectIds.size() > 0) {
			for (int i = 0; i < fromObjectIds.size(); i++) {
				/*
				 * DomainRelationship.connect(context, issueDomObj, strRelIssue, new
				 * DomainObject(fromObjectIds.get(i)));
				 */
	 ...
	 */
	@com.matrixone.apps.framework.ui.PostProcessCallable
	public void updateIssueDetails(Context context, String[] args) throws Exception {
		HashMap programMap = (HashMap) JPO.unpackArgs(args);
		HashMap paramMap = (HashMap) programMap.get("paramMap");
		HashMap requestMap = (HashMap) programMap.get("requestMap");
		String sIssueId = (String) paramMap.get("objectId");
		...
		if (!"User Agent".equalsIgnoreCase(strOriginator) && UIUtil.isNotNullAndNotEmpty(strOriginator)) {
			DomainAccess.createObjectOwnership(context, sIssueId, "", strOriginator + "_PRJ", "read,show",
					DomainAccess.COMMENT_MULTIPLE_OWNERSHIP, false);
		}
		if (UIUtil.isNotNullAndNotEmpty(strIssueCategoryClassification)) {
			isIssueCategorySelected = true;
			if (!"User Agent".equalsIgnoreCase(strOriginator) && UIUtil.isNotNullAndNotEmpty(strOriginator)) {
				DomainAccess.createObjectOwnership(context, sIssueId, "", strOriginator + "_PRJ",
						"toconnect,todisconnect,modify,changesov,changeowner", "TEMP OWNERSHIP", false);
			}
			updateIssueWithCategoryClassification(context, issObj, strIssueCategoryClassification,
					strIssueCategoryClassificationId);
		}
		...
		/*
		 * if (UIUtil.isNotNullAndNotEmpty(owner)) { StringList accessNames =
		 * DomainAccess.getLogicalNames(context, sIssueId); String fullAccess =
		 * (String)accessNames.get(accessNames.size()-1);
		 * DomainAccess.createObjectOwnership(context, sIssueId,
		 * "",owner+"_PRJ",fullAccess, DomainAccess.COMMENT_MULTIPLE_OWNERSHIP,false); }
		 */
		...
		if (isIssueCategorySelected) {
			if (!"User Agent".equalsIgnoreCase(strOriginator) && UIUtil.isNotNullAndNotEmpty(strOriginator)) {
				DomainAccess.deleteObjectOwnership(context, sIssueId, "", strOriginator + "_PRJ", "TEMP OWNERSHIP",
						true);
			}
		}
	}

	private void updateIssueWithCategoryClassification(Context context, Issue issObj,
			String strIssueCategoryClassification, String strIssueCategoryClassificationId) throws Exception {
		...
		strClassificationId = strIssueCategoryClassificationId.substring(iIndex + 1,
				strIssueCategoryClassificationId.length());
		...
		if (UIUtil.isNotNullAndNotEmpty(strCategoryId)) {
			...
			mlResults = categoryObj.getRelatedObjects(context, RELATIONSHIP_ISSUECATEGORYCLASSIFICATION,
					TYPE_ISSUECLASSIFICATION, true, true, 1, null, relselects, busWhere, null, 0, null, null, null);
	...
	private void setCoOwnersToIssue(Context context, String strCoOwners, Issue issue) throws Exception {
		 ...
		 * issue.updateIssueCoOwnerOwnership(context, issue.getObjectId(), new
		 * StringList(),newCoOwners);
		...
		MailUtil.sendNotification(context, newCoOwners, null, null,
				FrameworkStringResource.MultipleOwnerManager_AddCoOwnerSubject, subjectKeys, subjectValues,
				FrameworkStringResource.MultipleOwnerManager_AddCoOwnerMessage, messageKeys, messageValues,
				objectIdList, Company.getCompanyForKey(context, Person.getCompanyKey(context)).getName());
	...
	public void updateIssueClassification(Context context, String[] args) throws Exception {
		HashMap programMap = (HashMap) JPO.unpackArgs(args);
		HashMap paramMap = (HashMap) programMap.get("paramMap");
		String strObjId = (String) paramMap.get("objectId");
		Map requestMap = (Map) programMap.get("requestMap");
		...
		String[] strIssueCategoryClassification = (String[]) requestMap.get("Category Classification");
		String[] strIssueCategoryClassificationId = (String[]) requestMap.get("Category ClassificationOID");
		if (strIssueCategoryClassification.length == 1
				&& UIUtil.isNotNullAndNotEmpty(strIssueCategoryClassification[0])) {
			updateIssueWithCategoryClassification(context, issObj, strIssueCategoryClassification[0],
					strIssueCategoryClassificationId[0]);
		} else {
			Map attributeMap = new HashMap();
	...
	/**
	 * Gets the Category Classification This is used in the display of properties
	 * page of these objects, normally called from Webform.
	 * 
	 * @param context the eMatrix <code>Context</code> object.
	 * @param args    holds the following input arguments: 0 - String containing the
	 *                object id.
	...
	public String getCategoryClassification(Context context, String[] args) throws Exception {
		String displayValue = "";
		StringBuffer returnString = new StringBuffer();
	...
	private void connectIfResolvedTo(Context context, String strResolvedTo, String strParentOID, Issue issueDomObj)
			throws Exception {
	 ...
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    holds no arguments
	 * @return MapList containing the Object ids matching the search
	 *         criteria(Categories)
	 * @throws Exception if the operation fails
	 * @since AEF 418.HF3
	 */
	...
	public String getIssueMangersList(Context context, String[] args) throws Exception {
		...
		if (!iscloud) {
			issueManagerList = issueManagerList
					+ PropertyUtil.getSchemaProperty(context, Issue.SYMBOLIC_role_IssueManager) + ",";
		}
	...
	public MapList getExcludeIssueClassificationList(Context context, String[] args) throws Exception {
		...
		try {
			...
			// running the query
			MapList mapBusIds = findObjects(context, strType, null, null, null, DomainConstants.QUERY_WILDCARD,
					strWhereExpression, false, busSelects);
	...
	public MapList issueCategoryClassificationExpand(Context context, String[] args) throws FrameworkException {
		...
		try {
			...
			classlist = domCategory.getRelatedObjects(context,
					PropertyUtil.getSchemaProperty(context, SYMBOLIC_relationship_IssueCategoryClassification), strType,
					busSelects, relSelects, false, true, (short) 1, DomainConstants.EMPTY_STRING,
					DomainConstants.EMPTY_STRING);
	...
	public Vector getEditIcon(Context context, String[] args) throws Exception {
		...
		for (int i = 0; i < mapList.size(); i++) {
			...
			if (bExport || "false".equalsIgnoreCase(modifyAccess)) {
				...
				sbEditIcon.append(
						"', '570', '520')\"><img border='0' src='../common/images/iconActionEdit.gif'></img></a>");
	 ...
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains Context Issue Object, KINDOFOWNER, new owner,
	 *                old owner
	 * @return - int (0 or 1) 0 - If success and 1 - If blocked.
	 * @throws Exception if the operation fails
	 * @since 2018x.
	...
	public int updateOwnershipAccessonChangeOwner(Context context, String[] args) throws Exception {
		try {
			...
			if ("owner".equals(kindOfOwner)) {
				...
				if ("true".equalsIgnoreCase((String) busMap.get(DomainConstants.SELECT_HAS_CHANGESOV_ACCESS))) {
					DomainAccess.createObjectOwnership(context, strObjectId, newOwnerId, defaultAccess,
				...
				if (UIUtil.isNotNullAndNotEmpty(oldOwner)) {
					...
					if ("true".equalsIgnoreCase((String) busMap.get(DomainConstants.SELECT_HAS_CHANGESOV_ACCESS))
							&& oldOwner.equals(result)) {
						DomainAccess.deleteObjectOwnership(context, strObjectId, "", oldOwner + "_PRJ",
								DomainAccess.COMMENT_MULTIPLE_OWNERSHIP, false);
					}
				}
	...
	/**
	 * Method to check whether the person is owner or co-owner and showing the
	 * command
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries objectId -
	...
	/**
	 * To exclude the objects already added to the context object using 'Resolved
	 * To' relationship
	 * 
	 * @param context
	 * @param args
	 ...
	 */
	@com.matrixone.apps.framework.ui.ExcludeOIDProgramCallable
	public StringList getExcludeResolvedItemsList(Context context, String[] args) throws Exception {
		...
		relBusObjPageList = getRelatedObjects(context, strRelResolvedTo, DomainConstants.QUERY_WILDCARD, objectSelects,
				relSelects, true, true, recurseToLevel, DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
		...
		try {
			...
			if (type.equalsIgnoreCase(TYPE_ISSUE)) {
				...
				for (int i = 0; i < tempNewCoowners.size(); i++) {
					if (!tempOldCoowners.contains(tempNewCoowners.get(i))
							&& UIUtil.isNotNullAndNotEmpty(tempNewCoowners.get(i))) {
						listNewCoowners.add(tempNewCoowners.get(i));
					}
				}

				for (int i = 0; i < tempOldCoowners.size(); i++) {
					if (!tempNewCoowners.contains(tempOldCoowners.get(i))
							&& UIUtil.isNotNullAndNotEmpty(tempOldCoowners.get(i))) {
				...
				if (listOldCoowners.size() > 0) {
					for (int i = 0; i < listOldCoowners.size(); i++) {
						...
						for (Object objectAsReportedAgainst : reportedAgainstObjects) {
							...
							DomainAccess.deleteObjectOwnership(context, connectedID, "", personName + "_PRJ",
									commentForType, true);
						}
					}
				}
				...
				if (listNewCoowners.size() > 0) {
					for (int i = 0; i < listNewCoowners.size(); i++) {
						...
						for (Object objectAsReportedAgainst : reportedAgainstObjects) {
							...
							try {
								ContextUtil.pushContext(context, null, null, null);
								DomainAccess.createObjectOwnership(context, connectedID, "", personName + "_PRJ",
										lowestAccess, commentForType, false);
							} finally {
								ContextUtil.popContext(context);
							}
	...
	public boolean updateOwnershipForFollowers(Context context, String[] args) {
		try {
			...
			if ("true".equalsIgnoreCase((String) busMap.get(DomainConstants.SELECT_HAS_CHANGESOV_ACCESS))) {
				if (typeGroup.equalsIgnoreCase(fromType) || typeGroupProxy.equalsIgnoreCase(fromType)) {
					DomainAccess.createObjectOwnershipForUserGroups(context, toId, fromName, defaultAccess,
							COMMENT_MULTIPLE_OWNERSHIP_FOLLOWER);
				} else {
					DomainAccess.createObjectOwnership(context, toId, fromId, defaultAccess,
							COMMENT_MULTIPLE_OWNERSHIP_FOLLOWER);
				}
			}
			// StringList reportedAgainstObjects =
			// DomainAccess.getReportedAgainstObject(context, toId);
			// for(Object objectAsReportedAgainst : reportedAgainstObjects){
			// String connectedID = (String)objectAsReportedAgainst;
			// String lowestAccess = DomainAccess.getReaderAccessName(context, connectedID);
			// try {
			// ContextUtil.pushContext(context, null, null, null);
			// DomainAccess.createObjectOwnership(context, connectedID,"", fromName+"_PRJ",
			// lowestAccess, commentForType,false);
			// }finally{
			// ContextUtil.popContext(context);
			// }
	...
	public boolean clearOwnershipForFollowers(Context context, String[] args) {
		try {
			...
			if ("true".equalsIgnoreCase((String) busMap.get(DomainConstants.SELECT_HAS_CHANGESOV_ACCESS))) {
				if (typeGroup.equalsIgnoreCase(fromType) || typeGroupProxy.equalsIgnoreCase(fromType)) {
					DomainAccess.deleteObjectOwnership(context, toId, "", fromName, COMMENT_MULTIPLE_OWNERSHIP_FOLLOWER,
							false);
				} else {
					String cmd = "print role $1 select person dump";
					String result = MqlUtil.mqlCommand(context, cmd, fromName + "_PRJ");
					if (fromName.equals(result)) {
						DomainAccess.deleteObjectOwnership(context, toId, "", fromName + "_PRJ",
								COMMENT_MULTIPLE_OWNERSHIP_FOLLOWER, false);
					}
				}
			}
			...
			for (Object objectAsReportedAgainst : reportedAgainstObjects) {
				...
				try {
					ContextUtil.pushContext(context, null, null, null);
					DomainAccess.deleteObjectOwnership(context, connectedID, "", fromName + "_PRJ", commentForType,
							true);
				} finally {
					ContextUtil.popContext(context);
				}
	...
	public MapList getIssuesForWidgets(Context context, String[] args) throws Exception {
		Map programMap = (Map) JPO.unpackArgs(args);
		...
		// Forming the where expression
		// ((owner == context.user ) || (originator == context.user) || (to[Assigned
		// Issue].from.name == context.user) || (attribute[Co-Owners].value ~~
		// *context.user*)) AND (current != 'Closed')
		sbOwnerCondition.append(SYMB_OPEN_PARAN).append(SYMB_OPEN_PARAN).append(DomainConstants.SELECT_OWNER)
				.append(SYMB_EQUAL).append(SYMB_QUOTE).append(strOwner).append(SYMB_QUOTE).append(SYMB_CLOSE_PARAN)
				.append(SYMB_OR).append(SYMB_OPEN_PARAN).append(DomainConstants.SELECT_ORIGINATOR).append(SYMB_EQUAL)
				.append(SYMB_QUOTE).append(strOwner).append(SYMB_QUOTE).append(SYMB_CLOSE_PARAN).append(SYMB_OR);
	...
	/**
	 * Method to check for display of deprecated attributes under mql expression
	 * ISU_PROPERTIES.
	 *
	 * @param context - the eMatrix <code>Context</code> object
	 * @param args    - args contains a Map with the following entries objectId -
