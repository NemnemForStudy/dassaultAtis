...
<%@include file = "decImageManager.inc"%>
...
<script language="JavaScript" type="text/JavaScript">
    ...
    // Modified by jhlee on 2023.05.25 --- [S]
<%
	String sObjectId = emxGetParameter(request, "objectId");
	String imageType = emxGetParameter(request, "imageType");
	DomainObject doObj = DomainObject.newInstance(context, sObjectId);
	String sImageHolderModified = doObj.getInfo(context, "to["+ DecConstants.RELATIONSHIP_DECIMAGEHOLDER +"|from." + DecConstants.SELECT_ATTRIBUTE_DECIMAGETYPE + " == '" + imageType + "'].from.modified", false);
%>
function loadDate(){
	document.getElementById('divImage').innerHTML += "<div class='page-head' style='position: relative;bottom: 20px; left:250px;'><h2 style='color:yellow;'>Modified : <%=sImageHolderModified %></h2></div>";
}
    // Modified by jhlee on 2023.05.25 --- [E]
...
<!-- XSSOK -->
<body class="<%=strBodyClass%>" onload="showImage(<%=primaryImageIndex%>);adjustImageStyles();loadDate();">
  <div id="pageHeadDiv">
        <table>
            <tr>
                <td class="page-title">
		    <!-- XSSOK -->
 <!-- Modified by jhlee on 2023.05.25 --- [S] -->
                    <h2>Modified : <%=sImageHolderModified %></h2>
 <!-- Modified by jhlee on 2023.05.25 --- [E] -->
                </td>
            </tr>
        </table>    
        <jsp:include page = "../common/decemxToolbar.jsp" flush="true">
  ...
  <iframe class="hidden-frame" name="listDisplay" src="../common/emxBlank.jsp" HEIGHT="0" WIDTH="0"></iframe> 
    <form name="ImageViewerForm" method="post" >
    	<input type="hidden" name="imageType" value="<%=sImageType%>"/>
        <framework:ifExpr expr="<%=!hasImages%>">
            <table>
                <tr>
                    ...
                    <tr>
                        <framework:ifExpr expr="<%=showCheckbox%>">
                            ...
                            <td class="thumbnail" id="tdthumbnail">
                                <a href="javaScript:showImage(<%=i%>);loadDate();">
                                	<!-- XSSOK -->
