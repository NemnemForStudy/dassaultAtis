...
// END PAINTING    
    ...
    jsPlumbUtil.extend(jsPlumb.Connection, OverlayCapableJsPlumbUIComponent, {
        ...
        paint : function(params) {
            ...
            // GJA zoom management
            var currentZoom = this._jsPlumb.instance.getZoom();
            if(currentZoom > 0 && this._jsPlumb.paintStyleInUse) {
              if ( this._jsPlumb.paintStyleInUse.lineWidth) {
                if (!this._jsPlumb.paintStyleInUse.lineWidthOrig) {
                  this._jsPlumb.paintStyleInUse.lineWidthOrig = this._jsPlumb.paintStyleInUse.lineWidth;
                }
                this._jsPlumb.paintStyleInUse.lineWidth = this._jsPlumb.paintStyleInUse.lineWidthOrig * currentZoom;
              }
              if (this._jsPlumb.paintStyleInUse.outlineWidth) {
                if (!this._jsPlumb.paintStyleInUse.outlineWidth) {
                  this._jsPlumb.paintStyleInUse.outlineWidthOrg = this._jsPlumb.paintStyleInUse.outlineWidth;
                }
                this._jsPlumb.paintStyleInUse.outlineWidth = this._jsPlumb.paintStyleInUse.outlineWidthOrg * currentZoom;
              }
            }
            // end zoom management
