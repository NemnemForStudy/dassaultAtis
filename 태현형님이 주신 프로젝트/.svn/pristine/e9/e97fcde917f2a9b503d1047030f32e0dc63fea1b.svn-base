...
//End: Resume Process Implementation
        ...
        try {
            ...
            routeNodeObjectSelects.add("attribute["
                    + sAttAbsenceDelegate + "]");
			...
			.add(DomainConstants.SELECT_PHYSICAL_ID);
            routeNodeRelationshipSelects.add("attribute["
                    + sAttRouteSequence + "]");
            routeNodeRelationshipSelects.add("attribute["
                    + sAttRouteAction + "]");
            routeNodeRelationshipSelects.add("attribute["
            ...
            while (bGreaterSeqNoFound == 1) {
                ...
                for (int i = 0; i < nRoutedNodeObjectsSize; i++) {
                    ...
                    if (routeSequence == sRouteSequenceValue) {
                        ...
                        String initialTaskValue = (String) objectMap.get("attribute["
                                + INITIALTASK + "]");
                        ...
                        if (mlOldTasks.size() > 0) {
                            ...
                            // not touch them
                            //FUN131220                           
                            sTaskAutoComment =(String) mapTaskInfo.get("attribute["
                                    + sAttComment + "]");
                            sRejectedTask = (String) mapTaskInfo.get("attribute["
                                    + sAttApprovalStatus + "]");
                            ...
                            if("Complete".equals(taskCurrentState) && ( "Reject".equals(sRejectedTask)  || isAutoComment)) {
                            	//Do Nothing
                            }else {
