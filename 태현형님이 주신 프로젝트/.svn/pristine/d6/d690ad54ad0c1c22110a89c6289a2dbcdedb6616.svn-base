...
public class emxDnD_mxJPO extends emxDnDBase_mxJPO{
	...
	private MapList compareExcelIWPExcutionData(Context context, MapList mlObject, Map paramMap) throws Exception {
    	...
    	try ( SqlSession sqlSession = decSQLSessionFactory.getSession() ) {
    		...
    		String sErrorMsg8 = EnoviaResourceBundle.getProperty(context, "ProgramCentral","ProgramCentral.Alert.IsNotHaveIWPType", strLanguage) + "<br/>";
	    	...
	    	StringList slIWPType = new StringList();
			mParam.put("codeMasterName", "IWP Type");
			mlCodeDetail = codeJPO.getCodeDetailList(context, mParam);
	    	for(Object o : mlCodeDetail) {
	    		Map mDetail = (Map)o;
	    		String sIWPType = (String)mDetail.get(DecConstants.SELECT_NAME);
	    		slIWPType.add(sIWPType);
	    	}
			...
			SimpleDateFormat ev6FullSdf = new SimpleDateFormat("M/d/yyyy hh:mm:ss a");
	    	...
	    	for(Object obj : mlObject) {
		    	...
		    	if(!"Error".equals(sAction)) {
	    			...
	    			if(mCWPSubTask == null) {
						...
						String sIWPType = mColumn.get("IWP Type");
					    StringList slBOQ = mDisBOQ.get(sDiscipline.toUpperCase());
					    StringList slUOM = mBOQUOM.get(sIWPKeyType.toUpperCase());
					    if(slBOQ == null || !slBOQ.contains(sIWPKeyType.toUpperCase())) {
					    	sAction = "Error";
					    	sMessage += "IWP Key Q'ty Item" + sErrorMsg1;
					    }
					    if(slUOM == null || !slUOM.contains(sIWPKeyUOM.toUpperCase())) {
					    	sAction = "Error";
					    	sMessage += "UOM" + sErrorMsg2;
					    }
					    if(!slIWPType.contains(sIWPType.toUpperCase())) {
					    	sAction = "Error";
					    	sMessage += sErrorMsg8;
					    }
	...
	private MapList compareExcelCWPMasterData(Context context, MapList mlObject, Map paramMap) throws Exception{
    	...
    	try {
	    	...
	    	for(Object o : mlCWPActivityType) {
	    		Map mDetail = (Map)o;
	    		slCWPActivityType.add(((String)mDetail.getOrDefault(DecConstants.SELECT_ATTRIBUTE_DECCODE, "")).toUpperCase());
	    	}
	    	for(Object o : mlDiscipline) {
	    		Map mDetail = (Map)o;
	    		slDiscipline.add(((String)mDetail.getOrDefault(DecConstants.SELECT_ATTRIBUTE_DECCODE, "")).toUpperCase());
	    	}
			List<CompareRule> compareRuleList = new ArrayList<CompareRule>();
	    			...
	    			if(DecStringUtil.equalsIgnoreCase(sType, "CWP")){
        				...
        				if(!DecStringUtil.isAnyEmpty(sDiscipline, sCWPKeyType)) {
		        			...
		        			if(slBOQ == null) {
		        				sAction = "Error";
		        				sMessage += sErrorMsg8;
		        			}else if(!slBOQ.contains(sCWPKeyType.toUpperCase())) {
		        				sAction = "Error";
		        				sMessage += "CWP Key Q'ty Item" + sErrorMsg4;
		        			}
		        			if(DecStringUtil.isNotEmpty(sCWPKeyUOM)) {
			        			StringList slUOM = mBOQUOM.get(sCWPKeyType.toUpperCase());
			        			if(slUOM == null) {
			        				sAction = "Error";
			        				sMessage += sErrorMsg9;
			        			}else if(!slUOM.contains(sCWPKeyUOM.toUpperCase())) {
			        				sAction = "Error";
			        				sMessage += "UOM " + sErrorMsg5;
			        			}
			        			if(!slCWPActivityType.contains(sActivityType.toUpperCase())) {
			        				sAction = "Error";
			        				sMessage += sActivityType + " " + sErrorMsg15;
			        			}
			        			if(!slDiscipline.contains(sDiscipline.toUpperCase())) {
			        				sAction = "Error";
	...
	private MapList compareExcelKPIData(Context context, MapList mlObject, Map programMap) throws Exception {
		...
		try ( SqlSession sqlSession = decSQLSessionFactory.getSession() ) {
	    	...
	    	for(Object obj : mlObject) {
	    		...
	    		String sAction = DecConstants.EMPTY_STRING;
	    		String sMessage = DecConstants.EMPTY_STRING;
	    		StringList slBOQ = mDisBOQ.getOrDefault(sDiscipline.toUpperCase(), new StringList());
	    		if(DecStringUtil.isAnyBlank(sUnit, sDiscipline, sCItem, sWeek)) {
	    			sAction = "Error";
	    		}
	    		...
	    		if(DecStringUtil.isEmpty(sDiscipline)) {
	    			...
	    			sAction = "Error";
	    			sMessage += sDiscipline + " " + sErrorMsg4;
	    		}
	    		if(DecStringUtil.isEmpty(sCItem)) {
	    			sMessage += "Construction Item " + sErrorMsg1;
	    		}else if(!slBOQ.contains(sCItem.toUpperCase())) {
	    			sAction = "Error";
	    		...
	    		if(!"Error".equals(sAction)) {
	    			if(bAllCreate) {
	    	    		...
	    	    		String sKey = sProjectName+DecConstants.SYMB_UNDER_BAR+sUnitOID+DecConstants.SYMB_UNDER_BAR+sDiscipline+DecConstants.SYMB_UNDER_BAR+sCItem+DecConstants.SYMB_UNDER_BAR+sDate;
...
//		mColumn.put("settings", mSetting);
    ...
    public String importExcelCWPMasterData(Context context, String[] args)throws Exception{
    	...
    	try ( SqlSession sqlSession = decSQLSessionFactory.getSession() ) {
	    	for(Object obj : mlObject) {
	    		mColumn = (Map)obj;
	    		sActivityType = mColumn.get("Activity type").toUpperCase();
	    		sDiscipline = mColumn.get("Discipline").toUpperCase();
				...
				sCWPKeyType	 = mColumn.get("CWP Key Q'ty Item").toUpperCase();
    			sCWPKeyUOM	 = mColumn.get("UOM").toUpperCase();
    ...
    public String importExcelIWPExcutionData(Context context, String[] args)throws Exception{
    	...
    	try ( SqlSession sqlSession = decSQLSessionFactory.getSession() ) {
	    	for(Object obj : mlObject) {
	    		...
	    		sIWPType			 = mColumn.get("IWP Type").toUpperCase(); 
	    		...
	    		sSuperintendent	     = mColumn.get("Superintendent");
	    		sForeman			 = mColumn.get("Foreman"); 
	    		sIWPKeyType		     = mColumn.get("IWP Key Q'ty Item").toUpperCase();
	    		sIWPKeyUOM		     = mColumn.get("UOM").toUpperCase();
    ...
    public String importExcelConstructionKPIData(Context context, String[] args)throws Exception{
    	...
    	try ( SqlSession sqlSession = decSQLSessionFactory.getSession() ) {
	    	...
	    	for(Object obj : mlObject) {
	    		...
	    		sDisc =     mColumn.get("Discipline").toUpperCase();
	    		sCItem = 	mColumn.get("Construction Item").toUpperCase();
	    		sTarget = 	mColumn.get("Target Weekly");
	    		sActual = 	mColumn.get("Actual Weekly");
	    		sTotal = 	mColumn.get("Total Q'ty");
	    		sWeek = 	mColumn.get("Week").toUpperCase();
		    ...
		    for(Object o : mlDiscipline) {
		    	...
		    	for(int i=0; i<slBOQCurrent.size(); i++) {
		    		if(!"Active".equals(slBOQCurrent.get(i))) {
				    	...
				    	slBOQValidateName.add(slBOQName.get(i).toUpperCase());
				    }
		    	}
				mDisBOQ.put(sName.toUpperCase(), slBOQValidateName); 
		    }
	...
	private Map<String, StringList> getBOQValidateUOM (Context context, String sProjectName) throws Exception{
		...
		try {
		    ...
		    for(Object o : mlBOQKeyItem) {
		    	...
		    	for(int i=slUOMCurrent.size()-1; i>=0; i--) {
		    		if(!"Active".equals(slUOMCurrent.get(i))) {
		    			slUOMName.remove(i);
		    		}else {
		    			slUOMName.add(i, slUOMName.get(i).toUpperCase());
		    		}
		    	}
		    	mBOQUOM.put(sName.toUpperCase(), slUOMName); 
		    }
		}catch (FrameworkException e) {
			e.printStackTrace();
			throw e;
