...
<%
    ...
    selListObj.add(DomainConstants.SELECT_ATTRIBUTE_PLMCoreRepReference_V_isOnceInstantiable);
    selListObj.add(strKindOfPLMEntity);
...
<%
   ...
   isIRPCContent = (String)routableObjsMap.get(strKindOfPLMEntity);
...
%>
    <tr class='<framework:swap id ="1" />'>
      ...
      <td>
        ...
        %>
		<!-- //XSSOK -->
      <%=populateCombo(context, session, sStates, isAutoPromote, (String)routableObjsMap.get(routeObject.SELECT_ID), (String)routableObjsMap.get(routeObject.SELECT_RELATIONSHIP_ID), (String)routableObjsMap.get(routeObject.SELECT_OWNER), sCurState, sPolicy,sType, isIRPCContent, sRouteBaseState, sNoneValue, languageStr)%>
      </td>
    </tr>
...
<%!
    static public String populateCombo(matrix.db.Context context, HttpSession session,String sStates,boolean isAutoPromote, String routableObjId, String sRelationID, String routeOwner, String curState, String sPolicy, String sType, String isIRPCContent,String sStateValue, String sNoneValue,String sLangStr) {
      String sReturn    = "";
      ...
      try {
    	...
    	if((FrameworkUtil.hasAccess(context, routableObj, "promote")||isOwner) && com.matrixone.apps.common.Route.isStateBlockingAllowed(context,sType) && !"true".equalsIgnoreCase(isIRPCContent)){
