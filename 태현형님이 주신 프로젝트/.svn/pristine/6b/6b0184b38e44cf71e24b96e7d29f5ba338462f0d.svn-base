				...
				if (_draggable) {
					if (jsPlumb.isDragSupported(element, _currentInstance) && !jsPlumb.isAlreadyDraggable(element, _currentInstance)) {
						...
						options[dragEvent] = _ju.wrap(options[dragEvent], function() {
							...
							var ui = _currentInstance.getUIPosition(arguments, 1 /*_currentInstance.getZoom()*/);
							_draw(element, ui, null, true);
							_currentInstance.addClass(element, "jsPlumb_dragged");
						});
						options[stopEvent] = _ju.wrap(options[stopEvent], function() {
							var ui = _currentInstance.getUIPosition(arguments, 1 /*_currentInstance.getZoom()*/, true);
							_draw(element, ui);
					...
					var _drop = function() {
						...
						if (_continue) {
							...
							if (newEndpoint.anchor.positionFinder != null) {
								var dropPosition = this.getUIPosition(arguments, 1 /*this.getZoom()*/),
...
// --------------------- end makeSource/makeTarget ---------------------------------------------- 				
    ...
    jsPlumbUtil.extend(jsPlumbInstance, jsPlumbUtil.EventGenerator, {
		...
		getSize : function(el) {
      var z = this.getZoom();
			return [ el.offsetWidth * z, el.offsetHeight * z ];
		},
		getWidth : function(el) {
      var z = this.getZoom();	return el.offsetWidth * z ;
		},
		getHeight : function(el) {
			var z = this.getZoom(); return el.offsetHeight * z ;
		},
...
// maybe register static instance as an AMD module, and getInstance method too.
	if ( 0 && typeof define === "function") { //ZJX : temporary disable , as UWA_Swym_Alone_full.js 'define' also calls back getInstance,
                                            // which causes premature init without aother modules( renderers) loaded.
