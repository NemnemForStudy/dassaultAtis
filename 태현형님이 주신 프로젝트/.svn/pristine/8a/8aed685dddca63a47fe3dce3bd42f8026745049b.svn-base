...
try {
	...
	uomByDPMap.put("PP", new String[] {"DI","KG"});
	uomByDPMap.put("ME", new String[] {"EA","KG"});
	uomByDPMap.put("IC", new String[] {"LM"});
	uomByDPMap.put("EL", new String[] {"LM"});
	uomByDPMap.put("ST", new String[] {"KG"});
	...
	boolean isKG = uom.equalsIgnoreCase("KG");
	...
	if ( isKG )
	{
		...
		tempCompletedDoubleValue = Double.parseDouble(completedKg) + Double.parseDouble(completedTon) * 1000d;
			...
			for (Object obj : materialAllocationStatusList)
			{
				...
				if ( discipline.equals(tempDiscipline) // 공종이 paramater로 전달된 값과 같고
						&& ( uom.equals(keyQtyUOM) // 단위가 parameter로 전달된 값과 같거나
								|| ( isKG && "TON".equalsIgnoreCase(keyQtyUOM) ) ) // 단위가 Ton일 경우 KG도 허용
					)
				{
					if ( isKG )
					{
						// TON을 KG으로 변환
						if ( "TON".equalsIgnoreCase(keyQtyUOM) )
						{
...
function fnChangeUOM(dom) {
	...
	location.href = urlArr.join("&"); //보안 취약 조치
}
