...
;(function() {	
	...
	jsPlumb.Connectors.AbstractConnector = function(params) {
        ...
        */ 
        ...
        var _prepareCompute = function(params) {
          // GJA zoom management
          var currentZoom = 1;
          if (params && params.sourceEndpoint._jsPlumb && params.sourceEndpoint._jsPlumb.instance) {
            var instZoomValue = params.sourceEndpoint._jsPlumb.instance.getZoom();
            if (instZoomValue > 0)
              currentZoom = instZoomValue;
          } else if (params && params.targetEndpoint._jsPlumb && params.targetEndpoint._jsPlumb.instance) {
            var instZoomValue = params.targetEndpoint._jsPlumb.instance.getZoom();
            if (instZoomValue > 0)
              currentZoom = instZoomValue;
          }
          var zoomSourceGap = true;
          var zoomTargetGap = true;
          //if (params && params.targetEndpoint && params.targetEndpoint.endpoint && (params.targetEndpoint.endpoint.isResizable === false)) {
          //  zoomTargetGap = false;
          //}
          //if (params && params.sourceEndpoint && params.sourceEndpoint.endpoint && (params.sourceEndpoint.endpoint.isResizable === false)) {
          //  zoomSourceGap = false;
          //}
          // GJA end get currentZoom
            ...
            // GJA 
            var sx = swapX ? w + (sourceGap * (zoomSourceGap ? currentZoom : 1) * so[0]) : sourceGap * (zoomSourceGap ? currentZoom : 1) * so[0],
                sy = swapY ? h + (sourceGap * (zoomSourceGap ? currentZoom : 1) * so[1]) : sourceGap * (zoomSourceGap ? currentZoom : 1) * so[1],
                tx = swapX ? targetGap * (zoomTargetGap ? currentZoom : 1) * to[0] : w + (targetGap * (zoomTargetGap ? currentZoom : 1) * to[0]),
                ty = swapY ? targetGap * (zoomTargetGap ? currentZoom : 1) * to[1] : h + (targetGap * (zoomTargetGap ? currentZoom : 1) * to[1]),
                oProduct = ((so[0] * to[0]) + (so[1] * to[1]));        
             // GJA   
            ...
            var result = {
                ...
                segment : segment,
                startStubX : sx + (so[0] * sourceStub * currentZoom ), 
                startStubY : sy + (so[1] * sourceStub * currentZoom ),
                endStubX : tx + (to[0] * targetStub * currentZoom ), 
                endStubY : ty + (to[1] * targetStub * currentZoom ),
                isXGreaterThanStubTimes2 : Math.abs(sx - tx) > (sourceStub + targetStub) * currentZoom ,
                isYGreaterThanStubTimes2 : Math.abs(sy - ty) > (sourceStub + targetStub) * currentZoom ,
                opposite:oProduct == -1,
 ...
 // ********************************* ENDPOINT TYPES *******************************************************************
	...
	jsPlumb.Endpoints.Dot = function(params) {        
      ...
      // GJA - zoom management
		  var currentZoom = 1;
		  if (this && (this.isResizable === undefined || this.isResizable === true) && this._jsPlumb && this._jsPlumb.instance) {
		    var instanceZoom = this._jsPlumb.instance.getZoom();
		    if (instanceZoom > 0) {
		      currentZoom = instanceZoom;
		    }
		  }
		  // the radius is being read to compute the svg circle radius. 
		  // hence we need to modify the radius itself
		  if (this.radius) {
		    if (!this.radiusOrg) {
		      this.radiusOrg = this.radius;
		    }
		    this.radius = this.radiusOrg * currentZoom;
		  }
		  // end get zoom 
...
// ********************************* OVERLAY DEFINITIONS ***********************************************************************    
	...
	jsPlumb.Overlays.Arrow = function(params) {
  	  ...
  	  // GJA - zoom management
    	  var currentZoom = 1;
    	  if (this && this._jsPlumb && this._jsPlumb.instance) {
    	    var instanceZoom = this._jsPlumb.instance.getZoom();
    	    if (instanceZoom > 0) {
    	      currentZoom = instanceZoom;
    	    }
    	  }
        var loc_zoom = this.loc * currentZoom;
    	  var length_zoom = this.length * currentZoom;
    	  var width_zoom = this.width * currentZoom;
   	  // end get zoom 

            var hxy, mid, txy, tail, cxy;
            if (component.pointAlongPathFrom) {

                if (_ju.isString(loc_zoom) || loc_zoom > 1 || loc_zoom < 0) {                    
                    var l = parseInt(loc_zoom, 10),
                        fromLoc = loc_zoom < 0 ? 1 : 0;
                    hxy = component.pointAlongPathFrom(fromLoc, l, false);
                    mid = component.pointAlongPathFrom(fromLoc, l - (direction * length_zoom / 2), false);
                    txy = _jg.pointOnLine(hxy, mid, length_zoom);
                }
                else if (loc_zoom == 1) {                
					hxy = component.pointOnPath(loc_zoom);					           
                    mid = component.pointAlongPathFrom(loc_zoom, -(length_zoom));
					txy = _jg.pointOnLine(hxy, mid, length_zoom);
					...
					txy = component.pointOnPath(loc_zoom);                    
					mid = component.pointAlongPathFrom(loc_zoom, length_zoom);                    
					hxy = _jg.pointOnLine(txy, mid, length_zoom);                    
					if (direction == -1) {
						var __ = txy;
						txy = hxy;
						hxy = __;
    			    ...
    			    hxy = component.pointAlongPathFrom(loc_zoom, direction * length_zoom / 2);
                    mid = component.pointOnPath(loc_zoom);
                    txy = _jg.pointOnLine(hxy, mid, length_zoom);
                }

                tail = _jg.perpendicularLineTo(hxy, txy, width_zoom);
                cxy = _jg.pointOnLine(hxy, txy, foldback * length_zoom);    			
