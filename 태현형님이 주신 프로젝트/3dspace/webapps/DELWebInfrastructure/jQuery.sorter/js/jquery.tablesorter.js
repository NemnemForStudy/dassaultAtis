	...
	$.extend({
		...
		tablesorter: new function() {
			...
			ts.defaults = {
				...
				// *** advanced
				debug            : false,
				keepSelectionVisibleOnSort: true,
			...
			function bindEvents(table){
				...
				$this
				.unbind('sortReset update updateRows updateCell updateAll addRows sorton appendCache applyWidgetId applyWidgets refreshWidgets destroy mouseup mouseleave sortEnd '.split(' ').join('.tablesorter '))
				...
				.bind("updateCell.tablesorter", function(e, cell, resort, callback) {
					...
					if ($tb.length && (tbdy >= 0) && !$row.is('[data-tt-parent-id],.' + table.config.cssChildRow)) {
						//rowIndex = $tb.eq(tbdy).find('tr').index( $row );
						rowIndex = $tb.eq(tbdy).find('tr:not([data-tt-parent-id],.' + table.config.cssChildRow + ')').index( $row ); //use the same tr filter as was used when the cache was created!  see buildCache()
				...
				.bind("sortEnd.tablesorter", function(e){
					e.stopPropagation();
					//scroll up or down to keep selection visible
					if (table.config.keepSelectionVisibleOnSort)
						ts.makeSelectedLineVisible(table);
				});
			}
			...
			ts.makeSelectedLineVisible = function(table){
				if (table.config.debug){
					console.log("DEBUG -- make selected line visible!!");
				}

				var row; //selected row
				//retrieve first scrollable parent
				var scrollable = $(table).scrollParent();
    			//retrieve the selected rows
				var rows = $("tr.ui-selected", table.tBodies[0]);
                //there should be only one, otherwise we cannot scroll
                if (rows.size() > 0 && rows.size() < 2)
                {
                    row = rows[0];

                    if (rows.offset().top > scrollable.height())
                        row.scrollIntoView();
                }
				////scroll to position - ry9 - sometimes leads to weird scrolling values
    			//if (rows.size() > 0 && rows.offset().top > scrollable.height()){
				//	scrollable.scrollTop(rows.offset().top - (scrollable.offset().top));// - (scrollable.height()));
				//};
			}
