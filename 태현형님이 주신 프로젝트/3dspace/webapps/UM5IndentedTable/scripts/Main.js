...
function executeWidgetCode() {
    require([
        ...
        var myWidget = {
            ...
            loadRootData: function(oids, doInit, additionalParamObj) {
                if (typeof oids === "undefined") return;
                
                //CUSTOM START
                selects = widget.getValue("selects");
                myWidget.additionalParamObj = additionalParamObj;
                if ( additionalParamObj != undefined && additionalParamObj != null )
                {
                	var param = null;
                	var paramArr = null;
                	var targetKeyword = null;
                	var convertKeyword = null;
                	for (var k = 0; k < additionalParamObj.length; k++)
                	{
                		param = additionalParamObj[k];
                		paramArr = param.split("|");
                		if ( paramArr != null && paramArr.length >= 2 )
                		{
                			targetKeyword = paramArr[0];
                			convertKeyword = paramArr[1];
                			
                			try{
                				selects = selects.split(targetKeyword).join(convertKeyword);
                			}catch(e){
                				console.log(e);
                			}
                		}
                ...
                UM5ToolsWS.objInfo({
                    ...
                    onOk: function(data, callbackData) {
                        ...
                        {
                        	myWidget.dataFull = new Array();
                        	myWidget.oidsRoots = new Array();
...
//                            }
                        ...
                        if ( myWidget.additionalParamObj != undefined && myWidget.additionalParamObj != null )
                        {
                        	var arrConvertedData = new Array();
	                        for (var i = 0; i < arrDataObjs.length; i++) {
	                        	...
	                        	for (var keyIn in inObj) {
	                        		var convertedKeyIn = keyIn;
	                        		
	                        		for (var k = 0; k < myWidget.additionalParamObj.length; k++)
	                            	{
	                            		param = myWidget.additionalParamObj[k];
	                            		paramArr = param.split("|");
	                            		if ( paramArr != null && paramArr.length >= 2 )
	                            		{
	                            			targetKeyword = paramArr[0];
	                            			...
	                            			try{
	                            				convertedKeyIn = keyIn.split(convertKeyword).join(targetKeyword);
	                            			}catch(e){
	                            				console.log(e);
	                            			}
	                            		}
	                            	}
	                        		if ( convertedKeyIn != keyIn )
	                        		{
	                        			inObj[convertedKeyIn] = inObj[keyIn];
	                        		}
	                        	}
                            }
                        }
            ...
            addIds : function(paramJson) {
            	...
            	myWidget.loadRootData( ids, paramJson.doInit, paramJson.additionalParamObj );
            	for (var i in ids)
            	{
