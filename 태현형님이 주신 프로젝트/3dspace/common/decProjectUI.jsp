...
<emxUtil:localize id="i18nId" bundle="emxProgramCentralStringResource"
	locale='<xss:encodeForHTMLAttribute><%=request.getLocale().getLanguage()%></xss:encodeForHTMLAttribute>' />
...
<script src="../common/scripts/hichart/proj4.js"></script>
<script src="../common/scripts/hichart/jquery-ui.min.js"></script>
...
<link rel="stylesheet" type="text/css"
	href="../common/styles/emxUIForm.css">
<link href="../common/styles/jquery-ui.css" rel="stylesheet"
	type="text/css">
<link rel="stylesheet" type="text/css"
	href="../common/styles/decProjectUI.css">
...
table.form tr td.link {
	background: linear-gradient(rgb(245, 246, 247) 0%, rgb(226, 228, 227)
		100%);
}
...
<body>
	...
	String sGlobalUrl = decWebserviceUtil.getConnectionProperty("GlobeViewer.URL");
	...
	// Modified by choimingi on 2023.08.10 --- [s]
	String comStr = "com";
	String commonDashUrl = "";
	if (DecStringUtil.isNotNullString(sGlobalUrl)) {
		int comStrIndex = sGlobalUrl.indexOf(comStr);
		if (comStrIndex != -1) {
			// 찾은 위치에서부터 원하는 길이만큼 추출
			commonDashUrl = sGlobalUrl.substring(0, comStrIndex + 4);
			commonDashUrl += "3ddashboard";
		}
	}
	// Modified by choimingi on 2023.08.10 --- [e]
	...
	MapList countryList = JPO.invoke(context, "emxProjectSpace", null, "getCountryList", JPO.packArgs(new HashMap()),
			MapList.class);
	...
	MapList codeDetailList = JPO.invoke(context, "decCodeMaster", null, "getCodeDetailList", JPO.packArgs(programMap),
			MapList.class);
	...
	*/
	...
	String getSetting = MqlUtil.mqlCommand(context, "print command $1 select $2", "PMCProjectSpaceMyDesk",
			"setting[Registered Suite].value");
	...
	slRequestParam.add("to[" + DecConstants.RELATIONSHIP_DECPROJECTREQUESTREL + "].from.from[Member|"
			+ DecConstants.SELECT_ATTRIBUTE_DECPROJECTMEMBERROLE + "==PIM].to." + DecConstants.SELECT_NAME);
	...
	if (mlRequest.size() > 0 && context.isAssigned("decSystemAdmin")) {//확인해야 할 문의제안이 존재하고 SystemAdmin일 경우
		...
		for (Object obj : mlRequest) {
			...
			String PIMList = (String) mRequest.get("to[" + DecConstants.RELATIONSHIP_DECPROJECTREQUESTREL
			+ "].from.from[Member].to." + DecConstants.SELECT_NAME);
	...
	%>
	<script>
	window.addEventListener('resize', adjustDiv1Right);
	
	function adjustDiv1Right() {
	    var div2 = document.getElementById('projectInfoTable');
	    var div1 = document.getElementById('globeViewer');
	
	    var div2Width = div2.clientWidth;
	    var newRightValue = div2Width + 10; // div2의 크기보다 10px 더 큰 값
	
	    // div1의 right 값을 업데이트
	    div1.style.right = newRightValue + 'px';
	}
	
	</script>
	<div id="container"
		style="height: 100%; left: 0; top: 0; position: relative;"></div>
	<!-- 		style="height: 827px; width: calc(100% - 250px); left: 0; top: 0; position: relative;"></div> -->

	<div id="globeViewer"
		style="position: absolute; top: 0px; right: 0; width: 100px; height: 100px;">
		<a href="<%=sGlobalUrl%>" target="_blank"> <img
			src="../widget/images/MyApps/X3DGLOB_AP_AppIcon@2x.png" width="120px"
			height="120px" alt="Link"></a>
	</div>

	<table id="projectInfoTable" class="list form"
		style="position: absolute; top: 0; right: 0; height: 100%; width: 20%; max-width: 400px; margin: 0; background-color: white;">
		<tr>
			<th class="link" style="cursor: pointer; text-align: center;"
				onclick="javascript:window.open('<%=commonDashUrl%>')">
				<!--  img src="../widget/images/MyApps/X3DGLOB_AP_AppIcon@2x.png" style="height: 5vh; width: 5vh;"><br/-->
				<img src="../widget/images/MyApps/X3DDASH_AP_AppIcon.svg"
				style="height: 5vh; width: 5vh;"><br /> <span>Dashboard</span>
			</th>
			<th class="link" style="cursor: pointer; text-align: center;"
				onclick="javascript:top.location.href = '<%=div1projectUrl%>';">
				<img src="../widget/images/MyApps/ENOPRPR_AP_AppIcon.svg"
				style="height: 5vh; width: 5vh;"><br /> <span>Project
					List</span>
			</th>
		</tr>
		<tr>
			<th colspan="2" style="text-align: center;"><emxUtil:i18n
					localize="i18nId">emxProgramCentral.Label.ProjectStatus</emxUtil:i18n>
			</th>
		</tr>
		<!-- 		<tr> -->
		...
		<tr>
			<th colspan="2" style="text-align: center;"><emxUtil:i18n
					localize="i18nId">emxProgramCentral.Common.ProjectStatus</emxUtil:i18n>
				<!-- icon --></th>
		</tr>
		<tr>
			<td colspan="2" id="imageHolder"
				style="height: 27%; text-align: center;"></td>
		</tr>
		<tr>
			<th class="link" id="decProjectDetail"
				style="cursor: pointer; text-align: center;"><img
				id="decProjectDetailImage"
				src="../widget/images/MyApps/ENOMABO_AP_AppIcon.png"
				style="height: 5vh; width: 5vh;"><br /> <span id="textHolder">Detail</span>
			</th>
			<th class="link" id="decProjectDashboard"
				style="cursor: pointer; text-align: center;"><img
				src="../widget/images/MyApps/X3DDASH_AP_AppIcon.svg"
				style="height: 5vh; width: 5vh;"><br /> <span>PJ
					Dashboard</span></th>
		</tr>
		<tr>
...
// 							dashboardClick = "javascript:window.open('" + dashboardURL + "')";
							dashboardClick = "javascript:window.open('" + dashboardURL.htmlEncode() + "')";
						}
			        }
