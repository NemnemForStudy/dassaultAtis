...
<%
  ...
  try
  {
	...
	if ( "type_ProjectSpace".equals(objTypeName) )
	{
		// Member 여부 체크
		ProjectSpace projectSpace = new ProjectSpace(objectId);
		if ( !projectSpace.isCtxUserProjectMember(context) ) 
		{
%>
			alert("<%=EnoviaResourceBundle.getProperty(context, "emxProgramCentralStringResource", context.getLocale(), "emxProgramCentral.Msg.ProjectMemberCanAcesss") %>");
			if ( top.opener )
			{
				top.close();
			}
			...
			{
				if ( top.location.href.indexOf("objectId") > -1 ) 
				{
					top.location.href = "../common/emxNavigator.jsp";
				}
				else
				{
					top.location.reload();
...
<% if("type_decCWPTask".equals(mappedTreeName)){%>
<script type="text/javascript" src="../common/scripts/jquery-latest.js"></script>
<script>
if (getTopWindow().jQuery("div#treeBar")) {
	getTopWindow().jQuery("div#treeBar").hide();
}
getTopWindow().jQuery("button#catButton").removeClass("toggle-inactive");
getTopWindow().jQuery("button#catButton").addClass("toggle-active");
getTopWindow().jQuery("button#strucButton").removeClass("toggle-active");
getTopWindow().jQuery("button#strucButton").addClass("toggle-inactive");
getTopWindow().jQuery("div#catMenu").show();
getTopWindow().jQuery("div#leftPanelTree").hide();
</script>
<%}%>
</head>
</html>
