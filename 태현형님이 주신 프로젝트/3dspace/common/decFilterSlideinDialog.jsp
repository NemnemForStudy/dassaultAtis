...
function fnApplyURL2QueryString(urlArr, filterParamKeyArr, filterParamValueArr) {
	for (let k = 0; k < filterParamKeyArr.length; k++)
	{
		if ( filterParamValueArr[k] !== null )
		{
			...
			if ( fnFilterApplied( filterParamValueArr[k] ) )
			{
				filterApplied = true;
			}
		}
...
<%-- 	let openerFrame = findFrame(getTopWindow(), "<%=openerFrame %>"); --%>
	...
	filterApplied = false; // global variable
	...
	if ( filterParamKey.length > 0 )
	{
		...
		for (let k = 0; k < originalURLArr.length; k++)
		{
			...
			if ( tempQueryKey.indexOf("kw_") > -1 || tempQueryKey === "filterApplied" )
			{
				originalURLArr[k] = "";
				continue;
			}
			...
			if ( filterQueryKeyIdx > -1 )
			{
				if ( filterParamValue[filterQueryKeyIdx] !== null )
				{
					...
					if ( fnFilterApplied( filterParamValue[filterQueryKeyIdx] ) )
					{
						filterApplied = true;
					}
		...
		if ( filterApplied ) 
		{
			redirectURL += "&filterApplied=true";
		}
...
function fnFilterApplied(value) {
...
hr {
	margin: 0px;
}/* jhlee Add 2023-11-07 목록 필터 화면에서 안삐져나오게*/
.ms-drop.bottom{
	width: 100%;
}
.ms-drop.bottom > ul{
...
// 		String queryStr = emxGetQueryString(request);
		...
		boolean bKPI = false;
		boolean bMaterialStatus = false;
		boolean bCWPCmd = false;
		boolean bIWPCmd = false;
		for ( String queryExpr : queryExprList ){
			...
			if ("KPI".equals(queryKey) && "true".equals(queryValue)){
				bKPI = true;
				break;
			}else if("Material_Status".equals(queryKey) && "true".equals(queryValue)){
				bMaterialStatus = true;
				break;
			}else if("CWPCmd".equals(queryKey) && "true".equals(queryValue)){
				bCWPCmd = true;
				break;
			}else if("IWPCmd".equals(queryKey) && "true".equals(queryValue)){
				bIWPCmd = true;
				break;
			}
		}
				...
				for ( String codeMasterSplitByPipeline : slCodeMasterSplitByPipeline )
				{
					...
					for (int k = 0; k < slCodeMasterSplitByGT.size(); k++)
					{
						...
						else
						{
							...
							// jhlee Add 2023-10-24 --[s]
							if(bMaterialStatus && "Group".equals(codeMaster)){
								programMap.put("codeMasterName", "Discipline");
								programMap.put("codeDetailType", codeMasterTemp);
								programMap.put("expandLevelParam", 0);
								programMap.put("groupBySelect", DecConstants.SELECT_ATTRIBUTE_DECCODEDETAILTYPE);
								programMap.put("groupByValue", "Discipline");
								programMap.put("groupByDetailSelect", DecConstants.SELECT_ATTRIBUTE_DECCODEDETAILTYPE);
								programMap.put("groupByDetailValue", codeMasterTemp);
							}
							// jhlee Add 2023-10-24 --[e]
						}

				//		codeMasterList = JPO.invoke(context, "decCodeMaster", null, "getCodeDetailList", JPO.packArgs(programMap), MapList.class);
						// jhlee Add 2023-10-24 --[s]
						if(bMaterialStatus && "Group".equals(codeMaster)){
							Map<String, MapList> mGroupBy = JPO.invoke(context, "decCodeMaster", null, "getCodeDetailListGroupBy", JPO.packArgs(programMap), Map.class);
							codeMasterList = (MapList)mGroupBy.get(discipline);
						}else{
							codeMasterList = JPO.invoke(context, "decCodeMaster", null, "getCodeDetailList", JPO.packArgs(programMap), MapList.class);
						}
						// jhlee Add 2023-10-24 --[e]
						...
						// jhlee Add 2023-10-24 --[s]
						if(bKPI && "BOQ Key Item".equalsIgnoreCase(codeMaster)){
					...
					for (String codeMaster2HTML : slCodeMaster2HTML)
					{
						...
						{
			%>		<tr>
			<%			}// jhlee Add 2023-11-07 CWP/IWP중 하나만 보이게
			%>			<td class="label"><%=bCWPCmd && codeMaster2HTML.contains("CWP/IWP") ? codeMaster2HTML.replace("CWP/IWP", "CWP") : bIWPCmd && codeMaster2HTML.contains("CWP/IWP") ? codeMaster2HTML.replace("CWP/IWP", "IWP") : codeMaster2HTML %></td>
						<td class="field">
...
<%-- 									<input type="hidden" id="<%=fromToDateFilterName %>_End_msvalue" name="<%=fromToDateFilterName %>_End_msvalue" value="<%=(long) fromToDateFilterMap.getOrDefault("tomsvalue", 0l) %>" /> --%>
		...
		<%
			...
			else if ( "PO_No".equals(queryKey) )
			{
				...
				isMultiple = "true";
				// jhlee Add 2023-11-07 PO No 보여주는 값이 너무 길어 가로 한줄 전부다 사용하게 변경
				// tdIdx++;
				if ( tdIdx%2 == 1 )
				{
			//	<tr>
