...
$(function(){
	   /* 파일용량 제한*/
	  $("input[name=captureFile").on("change", function(){
	    let maxSize = 500 * 1024 * 1024; //* 500MB 사이즈 제한
			let fileSize = this.files[0].size; //업로드한 파일용량

	    if(fileSize > maxSize){
	    	alert("<emxUtil:i18nScript localize="i18nId">emxProgramCentral.Alert.ExceedFilesLimit</emxUtil:i18nScript>");
			$(this).val(''); //업로드한 파일 제거
			return; 
		}
	  });
