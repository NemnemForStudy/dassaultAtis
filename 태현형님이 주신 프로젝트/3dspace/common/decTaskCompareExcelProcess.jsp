...
<%
	...
	try{
		...
		String sPageHeading = request.getParameter("PageHeading");
		// Save file on disk
		...
		Workbook excelFile = DecExcelUtil.getExcelWorkBook(outfile.getPath());
        Sheet sheet = excelFile.getSheetAt(0);
		sbReturn.append("./decDashboardemxIndentedTable.jsp?table=decCwpTaskCompareExcelTable&program=emxDnD:getExcelData&freezePane=ActionCnt,Row,Action,Message");
		sbReturn.append("&maxCellsToDraw=2000&scrollPageSize=50&showPaginationToggle=false&suiteKey=ProgramCentral&StringResourceFileId=emxProgramCentralStringResource&SuiteDirectory=programcentral");
		sbReturn.append("&header=");
		sbReturn.append(sPageHeading);
		sbReturn.append("&fileName=");
		sbReturn.append(XSSUtil.encodeForURL(outfile.getName()));
		sbReturn.append("&").append(request.getQueryString()); // Added by hslee on 2023.06.23
	} catch (Exception e) {
		e.printStackTrace();
		throw e;
	}
