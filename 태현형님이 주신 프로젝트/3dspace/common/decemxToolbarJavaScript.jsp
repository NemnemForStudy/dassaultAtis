...
<!-- Modified by hslee on 2023.05.11 --- [s] -->
...
<jsp:useBean id="structureCompareBean" class="com.matrixone.apps.framework.ui.decUIStructureCompare" scope="session"/>
<!-- Modified by hslee on 2023.05.11 --- [e] -->
<%!
...
// Modified by hslee on 2023.05.11
// 	indentedTableBean = (com.matrixone.apps.framework.ui.UIStructureCompare)structureCompareBean;
	indentedTableBean = (com.matrixone.apps.framework.ui.decUIStructureCompare)structureCompareBean;
}
...
String disableToolbar = emxGetParameter(requestMap,"disableToolbar");// Modified by thok 2023.11.06
...
<%if("structureBrowser".equals(uiType)) {
	...
	if(!UIUtil.isNullOrEmpty(sSBDisplayView)) {
	    ...
	    } catch(MatrixException ex){ // 보안 취약 조치
...
//Added by hslee on 2023.05.25 --- [s]
String slideinFilter = emxGetParameter(requestMap, "slideinFilter");
String filterApplied = emxGetParameter(requestMap, "filterApplied");
String queryStr = request.getQueryString();
//queryStr += "&openerFrame=content";
//System.out.println(requestMap);
if( "true".equalsIgnoreCase(slideinFilter) )
{
 String toolTipFilter = UINavigatorUtil.getI18nString("emxPlmOnline.label.Search", "emxFrameworkStringResource", language);
 if("structureBrowser".equals(uiType)) {
	 String slideinFilterIcon = "iconActionSearchSpyGlass.png";
	 if ( "true".equalsIgnoreCase(filterApplied)) {
		 slideinFilterIcon = "iconActionSearchSpyGlassApplied.png";
	 }
%>
<%-- 		toolsMenu.addItem(new emxUIToolbarMenuItem(emxUIToolbar.ICON_ONLY, "iconActionSearchSpyGlass.png", "<%=toolTipFilter%>", "javascript:getTopWindow().showSlideInDialog(\"\", false, \"content\", \"right\", 500)")); --%>
		objContextToolbar.addItem(new emxUIToolbarButton(emxUIToolbar.ICON_ONLY, "<%=slideinFilterIcon %>", "<%=toolTipFilter%>", "javascript:showFilterSlideinDialog(\"<%=queryStr %>\")"));
<%
 }
}
//Added by hslee on 2023.05.25 --- [e]
//Added by hslee on 2023.07.28 --- [s]
String decExport = emxGetParameter(requestMap, "decExport");
//queryStr += "&openerFrame=content";
//System.out.println(requestMap);
if( "true".equalsIgnoreCase(decExport) )
{
 String toolTipFilter = UINavigatorUtil.getI18nString("emxFramework.Range.Functional_Area.Exporting_to_Excel", "emxFrameworkStringResource", language);
 if("structureBrowser".equals(uiType)) {
%>
		objContextToolbar.addItem(new emxUIToolbarButton(emxUIToolbar.ICON_ONLY, "iconActionExcelExport.png", "<%=toolTipFilter%>", "javascript:fnExportData(\"<%=queryStr %>\")"));
<%
 }
}
//Added by hslee on 2023.07.28 --- [e]
...
// Added by hslee on 2023.11.07 --- [s]
if ( pagecontent && pagecontent.setFilterCmd )
{
	setTimeout(pagecontent.setFilterCmd, timeOutDelay);
}
// Added by hslee on 2023.11.07 --- [e]
}
...
<%
	//Modifed by thok 2023.11.06[S]
	if(disableToolbar!= null && "true".equalsIgnoreCase(disableToolbar)){
%>
	var testToolbar = document.getElementById("divToolbar");
	var children = testToolbar.childNodes;
	children[1].style.display = "none";
<%
	}
	//Modifed by thok 2023.11.06[E]
%>
