...
$(window).on('resize', function(){
	$('.left').css('height', document.documentElement.clientHeight*0.94 + 'px');//Detail 공간 남기기 위해 -40
	$('.left').css('width', document.documentElement.clientWidth*0.30 + 'px');
	$('.right').css('height', document.documentElement.clientHeight*0.94 + 'px');
	$('.right').css('width', document.documentElement.clientWidth*0.67 + 'px');
	$('#container').css('width', '100%');
});
function openWindowPop(url, name){
    var options = 'width=1505, height=633,location=no,status=no';
...
<body style="height:94%;">
	<div style="text-align:left; width:48%;height:auto;margin-left:5px;">
		<button type="button" onclick="javascript:openWindowPop('../common/images/Material_Status_Legend.png', 'Material Status Legend');">LEGEND</button>
	</div>
	<div style="text-align:right; width:50%;height:auto;"><!-- margin-top:10px;margin-bottom:10px; -->
	...
	<div style="width: 98%;">
		<div class="left">
			<table border="1" style="width: 100%;height: 100%;position:relative;top:50%;transform:translateY(-50%);">
				<tr>
				...
				<%
					for(int i=0;i<statusCount.size();i++){
						...
						Double round = 0.0;
						if(sum!=0){
							round =  Math.round(Integer.parseInt(String.valueOf(map.get("CNT")))*1.0/sum*100 * 10 ) / 10.0;
						}
				%>
				<tr>
