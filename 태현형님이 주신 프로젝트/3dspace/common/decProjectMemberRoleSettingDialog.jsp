...
<%
	...
	for(String sRowId : sArrTableRowId){
		  ...
		  try{
			  ...
			  %>
			  	<script>
			  		alert("<%=EnoviaResourceBundle.getProperty(context, "emxProgramCentralStringResource", context.getLocale(), "ProgramCentral.Alert.MemberRoleSetting")%>");
			  		window.close();
			  	</script>
			  <%
			  return;
...
<script language="Javascript" >
	...
	$(document).ready(function() {
		$('input[type=checkbox]').change(function() {
			var ename = this.name;
			var bEmpty = true;
			let checkboxs = document.getElementsByName(ename);
			...
			let emptyCheck = document.getElementsByName(ename+"_empty");
			if(bEmpty){
				emptyCheck[0].value = "true";
			}else{
				emptyCheck[0].value = "";
			}
		});
...
%>
	<input type="hidden" name="<%=sPersonOID%>_empty"/>
<%
	}
%>
</form>
