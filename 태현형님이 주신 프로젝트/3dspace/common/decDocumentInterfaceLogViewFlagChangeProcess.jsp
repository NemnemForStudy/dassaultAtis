...
%>
<script>
	const selectTdOdd = document.getElementsByClassName("root-node mx_altRow mx_rowSelected");
	const selectTdEven = document.getElementsByClassName("root-node even mx_rowSelected");
    var selectTdOddSize = selectTdOdd.length;
    var selectTdEvenSize = selectTdOdd.length;
	
    if(selectTdOddSize>0){
...
try
{
	...
	int columnRows = objectIds.length;
	int lastIndex = 0;
	...
	String cmdName = (String) requestMap.get("portalCmdName");
	requestMap.put("RequestValuesMap", reqValuesMap); 
	requestMap.put("language",language);
	HashMap indexedList2 = new HashMap();
	...
	{		
	  ...
	  //  indexedList2 = (HashMap)tableData.get("IndexedObjectList");
	...
	String originator = context.getUser();
	//String projectId = (String) requestMap.get("objectId");
	//DomainObject pjDom = new DomainObject(projectId);
	//String siteCd = pjDom.getName(context);
	for(int i = 0 ; i < columnRows ; i++){
		String row = objectIds[i];
		lastIndex = row.lastIndexOf("|");
		if (lastIndex != -1 && lastIndex + 1 < row.length()) {
		    String sRow = row.substring(lastIndex + 1);
		    System.out.println("선택된 row :"+sRow);
		    Object nestedTreeMap = indexedList.get(sRow);
		    System.out.println("nestedTreeMap:"+nestedTreeMap);
		    mObject = (Map)nestedTreeMap;
		    //int    gIfseq 	    =	   (int) mObject.get("ID");
		    String gSeq 	    = 	   String.valueOf(mObject.get("ID"));
		    String gIfflag		=      (String) mObject.get("IF_FLAG");
		    String gRevision 	= 	   (String) mObject.get("REVISION");
		    String gDocno 		=      (String) mObject.get("DOC_NO");
		    String gSitecd  	=      (String) mObject.get("SITE_CD");
		    String gDoctitle  	=      (String) mObject.get("DOC_TITLE");
		    
		    Map selectParamMap = new HashMap();
			selectParamMap.put("IF_SEQ",gSeq);
			selectParamMap.put("IF_FLAG",gIfflag);
			selectParamMap.put("REVISION",gRevision);
			selectParamMap.put("DOC_NO",gDocno);
			selectParamMap.put("SITE_CD",gSitecd);
			selectParamMap.put("DOC_TITLE",gDoctitle);
			
			 try ( SqlSession sqlSession = decSQLSessionFactory.getBatchSession() ){
					
				 if(DecStringUtil.equals(cmdName,"decDILogViewDeliverable")){
						sqlSession.delete("Interface.updateFlagDeliverable", selectParamMap);
						System.out.println("*****updateFlagDeliverable Finish****");
					 
				 }else{
					 	sqlSession.delete("Interface.updateFlagVendorPrint", selectParamMap);
						System.out.println("*****Delete updateFlagVendorPrint Finish****");
				 }
				 sqlSession.commit(); 	
			}catch(Exception e) {
				e.printStackTrace();
...
<%-- 			<%
				 if(DecStringUtil.equals(cmdName,"decDILogViewDeliverable")){ %>
					 <script> getTopWindow().refreshTablePage(); </script>
