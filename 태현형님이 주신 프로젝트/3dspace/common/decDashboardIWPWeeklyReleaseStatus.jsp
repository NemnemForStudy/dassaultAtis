...
<%
	...
	String sTypePatterns = DecConstants.TYPE_DECCWPTASK + "," + DecConstants.TYPE_DECIWPTASK;
	...
	slSubTaskParam.add(DecConstants.SELECT_ATTRIBUTE_DECRELEASEPLAN);
	...
	slSubTaskParam.add("to[" + DecConstants.RELATIONSHIP_SUBTASK +"].from." + DecConstants.SELECT_ATTRIBUTE_DECDISCIPLINE);
	slSubTaskParam.add("to[" + DecConstants.RELATIONSHIP_SUBTASK +"].from." + DecConstants.SELECT_ATTRIBUTE_DECSUBCONNO);
	slSubTaskParam.add("to[" + DecConstants.RELATIONSHIP_SUBTASK +"].from." + DecConstants.SELECT_ATTRIBUTE_DECPRIORITYGROUP);
	slSubTaskParam.add("to[" + DecConstants.RELATIONSHIP_SUBTASK +"].from." + DecConstants.SELECT_ATTRIBUTE_DECPRIORITYNO);
	slSubTaskParam.add("to[" + DecConstants.RELATIONSHIP_SUBTASK +"].from." + DecConstants.SELECT_NAME);
	
	StringList slPhaseParam = new StringList();
	slPhaseParam.add(DecConstants.SELECT_ID);
	slPhaseParam.add(DecConstants.SELECT_LEVEL);
	slPhaseParam.add("from[" + DecConstants.RELATIONSHIP_SUBTASK + "|to.type==" + DecConstants.TYPE_DECCWPTASK + "].to.from[" + DecConstants.RELATIONSHIP_SUBTASK + "|to.type==" + DecConstants.TYPE_DECIWPTASK + "].to");
	if(DecStringUtil.equals(sCheckType, DecConstants.TYPE_DECCWPTASK)){
...
// 				if(DecStringUtil.equalsIgnoreCase("DP Hierarchy", sName)){
		...
		for(Object o : mlPhase) {
    		mPhase = (Map)o;
    		slIsChildIWP = DecStringUtil.getStringListChangeObject(mPhase.get("from[" + DecConstants.RELATIONSHIP_SUBTASK + "].to.from[" + DecConstants.RELATIONSHIP_SUBTASK + "].to"));
    		if(!slIsChildIWP.isEmpty()) {
		...
		for(String[] sArrLVName : setDataKey){
	    	...
	    	for(Object o : mlSubTask) {
		    	...
		    	sReleasePlan = (String)mSubTask.get(DecConstants.SELECT_ATTRIBUTE_DECRELEASEPLAN);
		    	sReleaseForecast = (String)mSubTask.get(DecConstants.SELECT_ATTRIBUTE_DECRELEASEFORECAST);
		    	sReleaseActual = (String)mSubTask.get(DecConstants.SELECT_ATTRIBUTE_DECRELEASEACTUAL);
		    	
		    	sCWPNo = (String)mSubTask.get("to[" + DecConstants.RELATIONSHIP_SUBTASK +"].from." + DecConstants.SELECT_NAME);
		    	if(DecStringUtil.isNotEmpty(sCWPCheckNo) && !sCWPNo.equals(sCWPCheckNo)){
		    		continue;
		    	}
		    	
		    	sTaskDiscipline = (String)mSubTask.get("to[" + DecConstants.RELATIONSHIP_SUBTASK +"].from." + DecConstants.SELECT_ATTRIBUTE_DECDISCIPLINE);
		    	sTaskSubConNo = (String)mSubTask.get("to[" + DecConstants.RELATIONSHIP_SUBTASK +"].from." + DecConstants.SELECT_ATTRIBUTE_DECSUBCONNO);
		    	sTaskPriorityGroup = (String)mSubTask.get("to[" + DecConstants.RELATIONSHIP_SUBTASK +"].from." + DecConstants.SELECT_ATTRIBUTE_DECPRIORITYGROUP);
		    	sTaskPriorityNumber = (String)mSubTask.get("to[" + DecConstants.RELATIONSHIP_SUBTASK +"].from." + DecConstants.SELECT_ATTRIBUTE_DECPRIORITYNO);   
		    	sPlanStartDate = (String)mSubTask.get(DecConstants.SELECT_ATTRIBUTE_TASK_ESTIMATED_START_DATE);
		    	if(!DecStringUtil.equals(sType, DecConstants.TYPE_DECIWPTASK)) {
		    		continue;
	        	}
		    	// 검색 조건에 전부 맞는지 체크
		    	...
		    	String targetDate = (String)mSubTask.get(DecConstants.SELECT_ATTRIBUTE_DECTASKFORECASTSTARTDATE);
		    	if(DecStringUtil.isEmpty(targetDate)) {
					targetDate = sPlanStartDate;
				}
...
var mPlanWeekData = new Map([
	...
	['<%=sKey%>',[
		...
		%>
			'<%=key.replace("attribute[", "").replace("]", "")%>' : '<%=mWeekData.get(key)%>', 
			<%}%>
			},
		<%}%>
		]
...
var mPlanMonthData = new Map([
	...
	['<%=sKey%>',[
		...
		<%
			for(String key : setKey){
		%>
			'<%=key.replace("attribute[", "").replace("]", "")%>' : '<%=mMonthData.get(key)%>', 
			<%}%>
			},
		<%}%>
		]
                    ...
                    events: {
						click: function (event) {
							...
							for(var data of dataList){
								...
								var tdIWPNo	 = document.createElement('td');
...
<body>
	<div style="display:flow-root;">
		...
		<header id="popup" style="float: right; margin: 10px;">
			...
			IWP Execution&emsp;
			<a href=JavaScript:window.open("emxTree.jsp?DefaultCategory=decIWPExcutionList&objectId=<%=sObjectId%>");><img src="images/iconActionNewWindow.png" border="0"></a>
