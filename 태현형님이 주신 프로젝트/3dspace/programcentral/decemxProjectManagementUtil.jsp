...
<%
		...
		if ("true".equalsIgnoreCase(hasModifyAccess) && !isProjectBaselineTask && !ProgramCentralConstants.STATE_PROJECT_SPACE_HOLD_CANCEL_CANCEL.equalsIgnoreCase(projectState)) {
			// jhlee Add 2023-09-13 editLink 비활성화
...
<%
        ...
        // Modified by jhlee on 2023-07-27 tableMenu, programMenu 비활성화 [S]
            //    	urlBuilder.append("../common/emxIndentedTable.jsp?tableMenu=PMCWBSTableMenu");
                	urlBuilder.append("../common/emxIndentedTable.jsp?table=PMCWBSViewTable");
                }
	//	urlBuilder.append("&expandProgramMenu=PMCWBSListMenu");
		urlBuilder.append("&expandProgram=emxTask:getWBSSubtasks");
    	// Modified by jhlee on 2023-07-27 tableMenu, programMenu 비활성화 [E]
        ...
        // Modified by jhlee on 2023-07-27 postProcessJPO 변경 [S]
	//	urlBuilder.append("&postProcessJPO=emxTask:updateScheduleChanges");
        urlBuilder.append("&postProcessJPO=emxTask:decUpdateScheduleChangesOOTB");
        // Modified by jhlee on 2023-07-27 postProcessJPO 변경 [E]
	    ...
	    // Modified by jhlee on 2023-09-08 EditLink 비활성화 [S]
		//	urlBuilder.append("&editLink=true");
    	// Modified by jhlee on 2023-09-08 EditLink 비활성화 [E]
...
//  final String  RELATIONSHIP_RESOLUTION_PROJECT   = "Resolution Project";  //from
	 ...
	 %>
		...
		// Modified by jhlee on 2023-08-09 object type에 따라 폼 변경 [S]
		objectId = XSSUtil.encodeURLForServer(context, objectId);
		DomainObject doObj = DomainObject.newInstance(context, objectId);
		doObj.open(context);
		String sObjType = doObj.getTypeName();
		String sForm = "PMCProjectTaskViewForm";
		String sToolbar = "PMCProjectTaskToolbar";
		if(DecStringUtil.equals(sObjType, DecConstants.TYPE_DECCWPTASK)){
			sForm = "decPMCProjectCWPTaskViewForm";
			sToolbar = "decPMCProjectTaskToolbar";
		}else if(DecStringUtil.equals(sObjType, DecConstants.TYPE_DECIWPTASK)){
			sForm = "decPMCProjectIWPTaskViewForm";
			sToolbar = "decPMCProjectTaskToolbar";
		}else if(DecStringUtil.equals(sObjType, DecConstants.TYPE_PHASE)){
			sToolbar = "decPMCProjectTaskToolbar";
		}
		String strURL = "../common/emxForm.jsp?mode=view&form=" + sForm + "&formHeader=emxProgramCentral.Common.FormBasics&PrinterFriendly=true&HelpMarker=emxhelpwbsview&findMxLink=false&suiteKey=ProgramCentral&SuiteDirectory=programcentral&toolbar=" + sToolbar + "&Export=false&objectId="+ objectId;
		// Modified by jhlee on 2023-08-09 object type에 따라 폼 변경 [E]
%>
<script language="javascript" type="text/javaScript">
...
<%
		...
		if(flattenedView){
	        ...
	        // Modified by jhlee on 2023-07-27 portal 변경 [S]
			contentURL.append("portal=PMCSchedule");
	    	// Modified by jhlee on 2023-07-27 portal 변경 [E]
